...
func NewValidationTestSuite(testDataDir string) (*ValidationTestSuite, error) {
	...
	validationRules := []ValidationRule{
		MetadataConsistencyRuleAdapter(),
		TaskConsistencyRuleAdapter(),
		StructureConsistencyRuleAdapter(),
		TimestampConsistencyRuleAdapter(),
		ProgressConsistencyRuleAdapter(),
	}
	return &ValidationTestSuite{
		...
		validationRules:  AdaptValidationRules(validationRules),
	}, nil
}
...
func (vts *ValidationTestSuite) TestCorrectionAutomatique(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			require.NoError(t, err, "Initial validation should succeed") // Attempt auto-fix
			fixResult, err := vts.autoResolver.AutoFixIssuesWithCount(planPath, initialResult.Issues)
...
func (vts *ValidationTestSuite) TestConflictResolutionStrategies(t *testing.T) {
	strategies := []struct {
	...
	}{
		{
			...
			conflicts: []Conflict{
				{
					Type:          ConvertToString(TaskStatusConflict),
		...
		},
		{
			...
			conflicts: []Conflict{
				{
					Type:          ConvertToString(MetadataConflict),
		...
		},
		{
			...
			conflicts: []Conflict{
				{
					Type:          ConvertToString(StructureConflict),
	...
	for _, strategyTest := range strategies {
		t.Run(strategyTest.name, func(t *testing.T) {
			for _, conflict := range strategyTest.conflicts {
				resolution, err := strategyTest.strategy.Resolve(TransformConflict(conflict))
...
func (vts *ValidationTestSuite) TestValidationRulesCoverage(t *testing.T) {
	...
	for _, rule := range vts.validationRules {
		t.Run(fmt.Sprintf("Rule_%s", GetRuleName(rule)), func(t *testing.T) {
			// Create test plan that triggers this rule
			testPlan := vts.createPlanForRule(ConsistencyRuleToValidationRule(rule))

			issues, err := ValidateRuleWithContext(rule, context.Background(), testPlan)
			assert.NoError(t, err, "Rule validation should not error")

			// Each rule should detect at least one issue in its test plan
			assert.Greater(t, len(issues), 0, "Rule should detect issues in test plan") // Verify issue types are correct for this rule
			for _, issueI := range issues {
				// Convert interface{} to map to access fields
				issue, ok := issueI.(map[string]string)
				if ok {
					assert.Contains(t, issue["type"], GetNameHelper(rule),
						"Issue type should relate to rule name")
				}
			}

			vts.logger.Info("✅ Validation rule coverage test completed: %s", GetNameHelper(rule))
		})
	}
}
...
func (vts *ValidationTestSuite) runCoherenceScenario(t *testing.T, scenario CoherenceTestScenario) {
	...
	if scenario.ShouldAutoFix && len(result.Issues) > 0 {
		fixResult, err := vts.autoResolver.AutoFixIssuesWithCount(planPath, result.Issues)
...
Progression: 50%%
`, GetNameForValidationRule(rule), GetNameForValidationRule(rule))

	return vts.writeTestFile(fmt.Sprintf("test-plan-%s.md", GetNameForValidationRule(rule)), content)
}
