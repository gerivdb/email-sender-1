<#
.SYNOPSIS
    Script d'arrêt propre de n8n en utilisant le PID.

.DESCRIPTION
    Ce script arrête proprement n8n en utilisant le PID enregistré dans un fichier.
...
.EXAMPLE
    .\stop-n8n.ps1 -PidFile "custom.pid" -Force
...
if (-not (Test-Path -Path $PidFile)) {
    ...
    Write-Host "n8n n'est peut-être pas en cours d'exécution ou a été démarré sans gestion du PID." -ForegroundColor Yellow
...
try {
    ...
    Write-Host "Arrêt de n8n (PID: $pid)..." -ForegroundColor Yellow
