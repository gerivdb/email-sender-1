...
<#
.SYNOPSIS
    Exécute les tests unitaires simplifiés pour les modules MCP.
.DESCRIPTION
    Ce script exécute les tests unitaires simplifiés pour les modules MCPManager et MCPClient.
.EXAMPLE
    .\Run-SimpleTests.ps1
    Exécute les tests unitaires simplifiés.
.NOTES
    Version: 1.0.0
...
$testFiles = @(
    (Join-Path -Path $PSScriptRoot -ChildPath "unit\MCPManager.Simple.Tests.ps1"),
    (Join-Path -Path $PSScriptRoot -ChildPath "unit\MCPClient.Simple.Tests.ps1")
)
