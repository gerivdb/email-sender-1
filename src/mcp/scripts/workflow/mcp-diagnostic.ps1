...
if (Test-Path "email\gateway.exe.cmd") {
    Write-Host "✅ Le fichier email\gateway.exe.cmd existe" -ForegroundColor Green
    
    # Tester l'execution
    try {
        $gatewayOutput = & .\email\gateway.exe.cmd help 2>&1
        Write-Host "✅ Le script email\gateway.exe.cmd s'execute correctement" -ForegroundColor Green
    } catch {
        Write-Host "❌ Erreur lors de l'execution de email\gateway.exe.cmd: $_" -ForegroundColor Red
        Write-Host "   Solution: Verifiez le contenu du fichier et les permissions d'execution"
    }
} else {
    Write-Host "❌ Le fichier email\gateway.exe.cmd n'existe pas" -ForegroundColor Red
    Write-Host "   Solution: Creez le fichier email\gateway.exe.cmd avec le contenu suivant:"
    Write-Host "   @echo off"
    Write-Host "   powershell -ExecutionPolicy Bypass -File `"%~dp0..\D`" %*"
}

if (Test-Path "..\D") {
    Write-Host "✅ Le fichier ..\D existe" -ForegroundColor Green
} else {
    Write-Host "❌ Le fichier ..\D n'existe pas" -ForegroundColor Red
    Write-Host "   Solution: Creez le fichier ..\D avec le script PowerShell approprie"
}
