...
#Requires -RunAsAdministrator # Get-CimInstance pour CommandLine peut nécessiter des privilèges élevés
...
function Get-ProcessesWithCommandLineCim {
    ...
    try {
        ...
        Write-LogInternal -Level ERROR -Message "Erreur d'autorisation lors de la récupération des processus. Veuillez exécuter le script en tant qu'administrateur." -ErrorRecord $_
        throw # Re-lance l'exception pour arrêter le script si ErrorActionPreference=Stop
    } catch {
