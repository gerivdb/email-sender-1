# Script pour vérifier l'état des serveurs MCP (version améliorée)
...
# Fonction pour obtenir les processus avec leurs lignes de commande
function Get-ProcessWithCommandLine {
    $processes = @()
    
    try {
        # Utiliser WMIC pour obtenir les processus avec leurs lignes de commande
        $wmicOutput = & wmic process get processid,commandline /format:csv
        
        # Analyser la sortie CSV
        $lines = $wmicOutput -split "`r`n" | Where-Object { $_ -match ',' }
            ...
            if ($values.Count -ge 2) {
                ...
                for ($j = 0; $j -lt $header.Count; $j++) {
                    if ($j -lt $values.Count) {
                        $process | Add-Member -MemberType NoteProperty -Name $header[$j].Trim() -Value $values[$j].Trim('"')
                    }
                }
                $processes += $process
            }
        ...
        Write-Log "Erreur lors de l'obtention des processus : $_" -Level "ERROR"
    }
...
# Obtenir tous les processus avec leurs lignes de commande
Write-Host "Récupération des processus en cours..." -ForegroundColor Cyan
$allProcesses = Get-ProcessWithCommandLine

# 1. Vérifier le serveur MCP Filesystem
Write-Host "1. Vérification du serveur MCP Filesystem..." -ForegroundColor Cyan
$filesystemRunning = Test-McpServerRunning -ServerName "Filesystem" -AllProcesses $allProcesses
...
$githubRunning = Test-McpServerRunning -ServerName "GitHub" -AllProcesses $allProcesses
if ($githubRunning) {
...
$gcpRunning = Test-McpServerRunning -ServerName "GCP" -AllProcesses $allProcesses
if ($gcpRunning) {
...
$gatewayRunning = Test-McpServerRunning -ServerName "Supergateway" -AllProcesses $allProcesses
if ($gatewayRunning) {
...
$augmentRunning = Test-McpServerRunning -ServerName "Augment" -AllProcesses $allProcesses
if ($augmentRunning) {
...
$gdriveRunning = Test-McpServerRunning -ServerName "GDrive" -AllProcesses $allProcesses
if ($gdriveRunning) {
...
$augmentStandardRunning = Test-McpServerRunning -ServerName "Augment Standard" -AllProcesses $allProcesses
...
$augmentGatewayRunning = Test-McpServerRunning -ServerName "Augment Gateway" -AllProcesses $allProcesses
...
$augmentNotionRunning = Test-McpServerRunning -ServerName "Augment Notion" -AllProcesses $allProcesses
...
$augmentGitIngestRunning = Test-McpServerRunning -ServerName "Augment Git Ingest" -AllProcesses $allProcesses
...
$augmentGitHubRunning = Test-McpServerRunning -ServerName "Augment GitHub" -AllProcesses $allProcesses
