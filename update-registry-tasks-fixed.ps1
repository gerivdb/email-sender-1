...
if (Test-Path -Path $updateCheckboxesPath) {
    ...
    Write-Host "Fonction Update-ActiveDocumentCheckboxes améliorée importée." -ForegroundColor Green
} else {
    Write-Error "La fonction Update-ActiveDocumentCheckboxes améliorée est introuvable à l'emplacement : $updateCheckboxesPath"
    exit 1
}
