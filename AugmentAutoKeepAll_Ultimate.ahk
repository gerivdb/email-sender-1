...
; Combine plusieurs approches pour une fiabilité maximale
; ========================================================
...
ChangeDetectionMethod() {
    ...
    methods := ["Recherche globale", "Recherche dans la barre d'état", "Recherche précise", "Position fixe"]
    ShowTooltip("Méthode de détection: " . DetectionMethod . " - " . methods[DetectionMethod], 1500)
}
...
SetKeepAllPosition() {
    global KeepAllRelativeX, KeepAllRelativeY, LastFoundX, LastFoundY
...
CheckForKeepAllButton:
    ...
    ; Changer de méthode toutes les 20 tentatives si aucun bouton n'a été trouvé
    if (Mod(DetectionAttempts, 20) = 0 && LastClickTime = 0) {
        DetectionMethod := Mod(DetectionMethod, 4) + 1
        if (DebugMode)
            ShowTooltip("Changement automatique de méthode: " . DetectionMethod, 500)
    }
...
FindAndClickKeepAll(force := false) {
    ...
    global KeepAllRelativeX, KeepAllRelativeY, DetectionMethod
