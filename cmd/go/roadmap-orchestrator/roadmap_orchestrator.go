...
func RunOrchestrator(configPath, phaseToRun, outputReportPath string) (OrchestrationReport, error) {
	...
	dummyConfig := OrchestratorConfig{
		Phases: []PhaseConfig{
			{Name: "Phase 1: Audit Modules", Command: []string{"go", "run", "email_sender/cmd/go/dependency-manager/audit_modules", "--output-json", "development/managers/dependency-manager/reports/" + time.Now().Format("20060102-150405") + "/initial_go_mod_list.json", "--output-md", "development/managers/dependency-manager/reports/" + time.Now().Format("20060102-150405") + "/initial_module_audit.md"}, Enabled: true},
			{Name: "Phase 1: Scan Non-Compliant Imports", Command: []string{"go", "run", "email_sender/cmd/go/dependency-manager/scan_non_compliant_imports", "--output-json", "development/managers/dependency-manager/reports/" + time.Now().Format("20060102-150405") + "/non_compliant_imports.json", "--output-md", "development/managers/dependency-manager/reports/" + time.Now().Format("20060102-150405") + "/non_compliant_imports_report.md"}, Enabled: true},
			{Name: "Phase 2: Validate Monorepo Structure", Command: []string{"go", "run", "email_sender/cmd/go/dependency-manager/validate_monorepo_structure", "--output-json", "development/managers/dependency-manager/reports/" + time.Now().Format("20060102-150405") + "/monorepo_structure_validation.json"}, Enabled: true},
