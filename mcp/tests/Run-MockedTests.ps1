...
<#
.SYNOPSIS
    Exécute les tests unitaires avec mocks complets pour les modules MCP.
.DESCRIPTION
    Ce script exécute les tests unitaires avec mocks complets pour les modules MCPManager et MCPClient.
.EXAMPLE
    .\Run-MockedTests.ps1
    Exécute les tests unitaires avec mocks complets.
.NOTES
    Version: 1.0.0
...
$testFiles = @(
    (Join-Path -Path $PSScriptRoot -ChildPath "unit\MCPManager.Mocked.Tests.ps1"),
    (Join-Path -Path $PSScriptRoot -ChildPath "unit\MCPClient.Mocked.Tests.ps1")
)
