@echo off
echo ===================================
echo Demarrage de n8n avec les MCP organises
echo ===================================
echo.
...
echo [2] Verification des fichiers batch MCP...
if exist src\mcp\batch\mcp-standard.cmd (
    echo - mcp-standard.cmd : OK
) else (
    echo - mcp-standard.cmd : MANQUANT
)

if exist src\mcp\batch\mcp-notion.cmd (
    echo - mcp-notion.cmd : OK
) else (
    echo - mcp-notion.cmd : MANQUANT
)

if exist src\mcp\batch\gateway.exe.cmd (
    echo - gateway.exe.cmd : OK
) else (
    echo - gateway.exe.cmd : MANQUANT
)

if exist src\mcp\batch\mcp-git-ingest.cmd (
...
if exist .n8n\credentials.db (
    ...
    echo Executez le script scripts\setup\configure-n8n-mcp.ps1 pour configurer les identifiants MCP.
)
echo.
