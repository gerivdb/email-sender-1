...
<#
.SYNOPSIS
    Tests unitaires simples pour le module MCPClient.
.DESCRIPTION
    Ce script contient les tests unitaires simples pour le module MCPClient qui interagit avec le serveur FastAPI.
    Ces tests utilisent des mocks pour éviter de se connecter à un serveur réel.
.EXAMPLE
    Invoke-Pester -Path .\MCPClient.Tests.Simple.ps1
    Exécute les tests unitaires simples pour le module MCPClient.
.NOTES
    Version: 1.0.0
...
# Définir les tests
Describe "MCPClient Module Tests (Simple)" {
    BeforeAll {
