...
function Calculate-ROI {
    ...
    return @{
        ImmediateTimeSaved = $totalImmediate
        MonthlyTimeSaved = $totalMonthly
        AnnualTimeSaved = $totalMonthly * 12
        PhasesCompleted = $global:AutomationMetrics.PhasesCompleted.Count
        TotalErrors = $global:AutomationMetrics.ErrorsEncountered.Count
    }
}
    $totalMonthly = ($global:AutomationMetrics.TimesSaved.Values | ForEach-Object { $_.Monthly }) | Measure-Object -Sum | Select-Object -ExpandProperty Sum
...
function Invoke-InvertedTDD {
    ...
    try {
        ...
        if (Test-Path "internal/testgen/generator.go") {
            ...
            if (-not $DryRun) {
                # Generate tests for key components                $targets = @(
...
function Invoke-CodeGenerationFramework {
    Write-Info "⚙️ Phase 5: Implementing Code Generation Framework..."
        ...
        Write-Progress "Running code generation..."
          if (Test-Path "internal/codegen/generator.go") {
            ...
            if (-not $DryRun) {
                ...
                $result = go run cmd/codegen/main.go -type=cli "-commands=search,index,serve,metrics" -output=generated/cli 2>&1
                if ($LASTEXITCODE -eq 0) {
...
function Invoke-IntegrationTesting {
    if ($SkipTests) {
        Write-Info "⏭️ Skipping integration tests (--SkipTests flag)"
        return
    }
        ...
        if (-not $DryRun) {
            ...
            if ($LASTEXITCODE -eq 0) {
                ...
                $healthChecks = @(
                    ...
                    @{ Name = "Redis"; Command = "docker exec $(docker-compose ps -q redis) redis-cli ping" }
                )
...
| ⚙️ Code Generation Framework | $(if ('Phase5_CodeGeneration' -in $global:AutomationMetrics.PhasesCompleted) { '✅ Completed' } else { '❌ Failed' }) | +36 hours | - |
...
"@
    ...
    Write-Host "📊 Total ROI: " -ForegroundColor Cyan -NoNewline
    Write-Host "+$($roi.ImmediateHours)h immediate, +$($roi.YearlyHours)h yearly" -ForegroundColor Green
    Write-Host "💰 Dollar Value: " -ForegroundColor Cyan -NoNewline
    Write-Host "`$$($roi.DollarValue.Immediate) immediate, `$$($roi.DollarValue.Yearly) yearly" -ForegroundColor Green
    Write-Host ""
    
    if ($global:AutomationMetrics.ErrorsEncountered.Count -gt 0) {
        Write-Warning "⚠️ Errors encountered during automation:"
