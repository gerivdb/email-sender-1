...
if (Test-Path $testWorkflowPath) {
    ...
    Write-Host "1. Demarrez n8n avec la commande: ..\..\D" -ForegroundColor Yellow
    ...
    if ($startN8n -eq "O" -or $startN8n -eq "o") {
        ...
        Start-Process -FilePath "cmd.exe" -ArgumentList "/c", "..\..\D"
    } else {
        Write-Host "Vous pouvez demarrer n8n plus tard avec la commande: ..\..\D" -ForegroundColor Yellow
    }
} else {
