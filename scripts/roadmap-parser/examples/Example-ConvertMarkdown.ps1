...
.DESCRIPTION
    ...
    un fichier markdown en structure d'objet PowerShell et effectuer diverses opérations sur cette structure.
...
Ceci est un exemple de roadmap pour démontrer l'utilisation de la fonction ConvertFrom-MarkdownToObject.

## Planification

- [ ] **1** Analyse des besoins
  - [x] **1.1** Recueillir les exigences @john #important
  - [ ] **1.2** Analyser la faisabilité
    - [~] **1.2.1** Étude technique @date:2023-08-15
    - [!] **1.2.2** Évaluation des coûts P1

## Développement

- [ ] **2** Implémentation
  - [ ] **2.1** Développer le backend @sarah
  - [ ] **2.2** Créer l'interface utilisateur
"@ | Out-File -FilePath $exampleMarkdownPath -Encoding UTF8

Write-Host "Fichier markdown d'exemple créé: $exampleMarkdownPath" -ForegroundColor Green
...
Write-Host "`nExemple 2: Conversion avec extraction des métadonnées" -ForegroundColor Cyan
$roadmapWithMetadata = ConvertFrom-MarkdownToObject -FilePath $exampleMarkdownPath -IncludeMetadata

# Fonction pour afficher récursivement les items
function Show-Items {
    param (
    ...
    if ($Item.PSObject.Properties.Name -contains "Metadata" -and $Item.Metadata.Count -gt 0) {
        Write-Host "$indentation  Métadonnées:" -ForegroundColor Magenta
...
# Afficher la structure avec métadonnées
Write-Host "`nStructure de la roadmap avec métadonnées:" -ForegroundColor Green
...
# Fonction récursive pour trouver tous les items correspondant à un critère
function Find-Items {
    param (
        [Parameter(Mandatory = $true)]
        [PSCustomObject]$Item,
...
Write-Host "Tâches complétées:" -ForegroundColor Green
foreach ($task in $completedTasks) {
    Write-Host "- $($task.Title) (ID: $($task.Id))" -ForegroundColor Green
}

# Trouver toutes les tâches assignées à John
...
Write-Host "`nTâches assignées à John:" -ForegroundColor Yellow
foreach ($task in $johnsTasks) {
    Write-Host "- $($task.Title) (ID: $($task.Id))" -ForegroundColor Yellow
}

# Trouver toutes les tâches importantes
...
Write-Host "`nTâches importantes:" -ForegroundColor Magenta
...
Write-Host "- Nombre total de tâches: $($stats.TotalTasks)" -ForegroundColor White
Write-Host "- Tâches complétées: $($stats.CompleteTasks) ($($stats.CompletePercentage)%)" -ForegroundColor Green
Write-Host "- Tâches en cours: $($stats.InProgressTasks) ($($stats.InProgressPercentage)%)" -ForegroundColor Yellow
Write-Host "- Tâches bloquées: $($stats.BlockedTasks) ($($stats.BlockedPercentage)%)" -ForegroundColor Red
Write-Host "- Tâches incomplètes: $($stats.IncompleteTasks) ($($stats.IncompletePercentage)%)" -ForegroundColor Gray

Write-Host "`nExemples terminés. Vous pouvez explorer le fichier markdown et les résultats." -ForegroundColor Green
