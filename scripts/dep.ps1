...
# Usage: .\scripts\dep.ps1 [command] [options]
...
# Utilise le nouveau gestionnaire de dépendances unifié
$BinaryPath = ".\development\managers\dependency-manager\dependency-manager.exe"
...
function Show-Help {
   ...
   Write-Host "  .\scripts\dep.ps1 list"
   Write-Host "  .\scripts\dep.ps1 add github.com/pkg/errors v0.9.1"
   Write-Host "  .\scripts\dep.ps1 remove github.com/pkg/errors"
   Write-Host "  .\scripts\dep.ps1 update github.com/gorilla/mux"
}

function Build-DepManager {
   Write-Host "Compilation du gestionnaire de dépendances..." -ForegroundColor Yellow
   Set-Location "development\managers\dependency-manager\modules"
   try {
      go build -o "..\dependency-manager.exe" dependency_manager.go
...
function Invoke-DepManager {
   ...
   Set-Location "development\managers\dependency-manager"
   try {
      & ".\dependency-manager.exe" @Arguments
   }
   finally {
      Set-Location "..\..\..\"
   }
...
switch ($Command) {
   ...
   "add" {
      if ([string]::IsNullOrEmpty($Module)) {
         ...
         Write-Host "Usage: .\scripts\dep.ps1 add <module> [version]" -ForegroundColor Yellow
         exit 1
      }
   ...
   "remove" {
      if ([string]::IsNullOrEmpty($Module)) {
         ...
         Write-Host "Usage: .\scripts\dep.ps1 remove <module>" -ForegroundColor Yellow
         exit 1
      }
   ...
   "update" {
      if ([string]::IsNullOrEmpty($Module)) {
         ...
         Write-Host "Usage: .\scripts\dep.ps1 update <module>" -ForegroundColor Yellow
         exit 1
      }
