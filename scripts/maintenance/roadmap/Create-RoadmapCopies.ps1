...
param (
    ...
    [Parameter(Mandatory = $false)]
    [string]$LogFilePath = (Join-Path -Path (Get-Location) -ChildPath "logs\roadmap_copies.log")
)
...
try {
    ...
    # Définir les chemins des copies à créer
    $copyPaths = @(
    ...
    Write-Host "Copies créées : $($copyPaths.Count)" -ForegroundColor Green
