...
try {
    ...
    if (Test-Path $destinationPath) {
        if ($Force) {
            ...
            Write-Host "  [OK] $fileName deplace vers $DestinationFolder (remplace)" -ForegroundColor Green
        } else {
            Write-Host "  [ATTENTION] $fileName existe deja dans $DestinationFolder" -ForegroundColor Yellow
            ...
            if ($confirmation -eq "O" -or $confirmation -eq "o") {
                ...
                Write-Host "  [NON] $fileName conserve a son emplacement actuel" -ForegroundColor Red
            }
        }
    } else {
    ...
    if ($files.Count -eq 0) {
        ...
        foreach ($file in $files) {
            ...
            if ($file.Name -eq "README.md" -and $destination -ne ".") {
                Write-Host "  [INFO] README.md conserve a la racine" -ForegroundColor Blue
                continue
            }
...
"@
    ...
    Write-Host "`n[OK] Fichier .gitignore cree" -ForegroundColor Green
} else {
    Write-Host "`n[OK] Fichier .gitignore existe deja" -ForegroundColor Green
}
...
if (-not (Test-Path $readmePath)) {
    ...
    Write-Host "[OK] Fichier README.md cree" -ForegroundColor Green
} else {
    Write-Host "[ATTENTION] Un fichier README.md existe deja" -ForegroundColor Yellow
    Write-Host "Voulez-vous le remplacer par un README standardise ? (O/N)" -ForegroundColor Yellow
    $confirmation = Read-Host
    
    if ($confirmation -eq "O" -or $confirmation -eq "o") {
        Set-Content -Path $readmePath -Value $readmeContent
        Write-Host "[OK] Fichier README.md mis a jour" -ForegroundColor Green
    } else {
        Write-Host "[NON] README.md conserve tel quel" -ForegroundColor Red
    }
}
...
# Usage: ..\..\D -Type workflow -Name mon-workflow
...
if (-not (Test-Path $newFilePath)) {
    ...
    Write-Host "[OK] Script new-file.ps1 cree" -ForegroundColor Green
} else {
    Write-Host "[OK] Script new-file.ps1 existe deja" -ForegroundColor Green
}
