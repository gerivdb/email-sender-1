...
"@
    ...
    Context "Analyse de l'historique des corrections" {
        It "Devrait analyser l'historique des corrections avec succès" {
            ...
            $output = & $scriptPath -TrainingMode -ModelPath $modelPath -ErrorAction SilentlyContinue 6>&1
    ...
    Context "Chargement du modèle" {
        It "Devrait charger un modèle existant" {
            ...
            $output = & $scriptPath -ModelPath $modelPath -ErrorAction SilentlyContinue 6>&1
    ...
    Context "Application du modèle" {
        It "Devrait tester le modèle sur un script" {
            ...
            $output = & $scriptPath -TestScript $scriptToTest -ModelPath $modelPath -ErrorAction SilentlyContinue 6>&1
