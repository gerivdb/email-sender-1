...
if (Test-Path ".env") {
    ...
    if ($hasEnvVar) {
        Write-Host "âœ… La variable est correctement definie dans le fichier .env" -ForegroundColor Green
    } else {
        Write-Host "âŒ La variable n'est pas definie dans le fichier .env" -ForegroundColor Red
        Write-Host "   Solution: Ajoutez la ligne suivante au fichier .env:"
        Write-Host "   N8N_COMMUNITY_PACKAGES_ALLOW_TOOL_USAGE=true"
    }
} else {
    Write-Host "âŒ Le fichier .env n'existe pas" -ForegroundColor Red
...
if ($hasMcpPackage) {
    Write-Host "âœ… Le package n8n-nodes-mcp est installe" -ForegroundColor Green
} else {
    Write-Host "âŒ Le package n8n-nodes-mcp n'est pas installe" -ForegroundColor Red
    Write-Host "   Solution: Installez le package avec la commande:"
    Write-Host "   npm install n8n-nodes-mcp"
}

if ($hasNotionPackage) {
    Write-Host "âœ… Le package @suekou/mcp-notion-server est installe" -ForegroundColor Green
} else {
    Write-Host "âŒ Le package @suekou/mcp-notion-server n'est pas installe" -ForegroundColor Red
    Write-Host "   Solution: Installez le package avec la commande:"
...
if (Test-Path "gateway.exe.cmd") {
    Write-Host "âœ… Le fichier gateway.exe.cmd existe" -ForegroundColor Green
    
    # Tester l'execution
    try {
        $gatewayOutput = & .\gateway.exe.cmd help 2>&1
        Write-Host "âœ… Le script gateway.exe.cmd s'execute correctement" -ForegroundColor Green
    } catch {
        Write-Host "âŒ Erreur lors de l'execution de gateway.exe.cmd: $_" -ForegroundColor Red
        Write-Host "   Solution: Verifiez le contenu du fichier et les permissions d'execution"
    }
} else {
    Write-Host "âŒ Le fichier gateway.exe.cmd n'existe pas" -ForegroundColor Red
...
if (Test-Path "gateway.ps1") {
    Write-Host "âœ… Le fichier gateway.ps1 existe" -ForegroundColor Green
} else {
    Write-Host "âŒ Le fichier gateway.ps1 n'existe pas" -ForegroundColor Red
    Write-Host "   Solution: Creez le fichier gateway.ps1 avec le script PowerShell approprie"
}

if (Test-Path "gateway.yaml") {
    Write-Host "âœ… Le fichier gateway.yaml existe" -ForegroundColor Green
} else {
    Write-Host "âŒ Le fichier gateway.yaml n'existe pas" -ForegroundColor Red
...
if ($executionPolicy -eq "Restricted") {
    Write-Host "âŒ La politique d'execution PowerShell est restrictive" -ForegroundColor Red
    Write-Host "   Solution: Modifiez temporairement la politique d'execution avec la commande:"
    Write-Host "   Set-ExecutionPolicy -Scope Process -ExecutionPolicy Bypass"
} else {
    Write-Host "âœ… La politique d'execution PowerShell permet l'execution de scripts" -ForegroundColor Green
}
