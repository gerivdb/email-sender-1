# Script pour vérifier l'état des serveurs MCP (version alternative)
...
function Test-McpServerRunning {
    ...
    if ($patterns.ContainsKey($ServerName)) {
        ...
        # Utiliser tasklist pour obtenir les processus
        $output = tasklist /v /fo csv | ConvertFrom-Csv
        
        foreach ($process in $output) {
            # Vérifier si le processus est node.exe ou cmd.exe
            if (($process."Image Name" -eq "node.exe" -or $process."Image Name" -eq "cmd.exe") -and 
                ($process."Window Title" -match $pattern)) {
                return $true
            }
        }
    }
...
$filesystemRunning = Test-McpServerRunning -ServerName "Filesystem"
...
$githubRunning = Test-McpServerRunning -ServerName "GitHub"
if ($githubRunning) {
...
$gcpRunning = Test-McpServerRunning -ServerName "GCP"
if ($gcpRunning) {
...
$gatewayRunning = Test-McpServerRunning -ServerName "Supergateway"
if ($gatewayRunning) {
...
$augmentRunning = Test-McpServerRunning -ServerName "Augment"
if ($augmentRunning) {
...
$gdriveRunning = Test-McpServerRunning -ServerName "GDrive"
if ($gdriveRunning) {
...
$augmentStandardRunning = Test-McpServerRunning -ServerName "Augment Standard"
...
$augmentGatewayRunning = Test-McpServerRunning -ServerName "Augment Gateway"
...
$augmentNotionRunning = Test-McpServerRunning -ServerName "Augment Notion"
...
$augmentGitIngestRunning = Test-McpServerRunning -ServerName "Augment Git Ingest"
...
$augmentGitHubRunning = Test-McpServerRunning -ServerName "Augment GitHub"
