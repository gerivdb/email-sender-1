...
Multi-File SupportThe read_file tool can read multiple files simultaneously when the maxConcurrentFileReads setting is greater than 1. This significantly improves efficiency for tasks requiring analysis of multiple related files.
...
Legacy FormatWhile the single-file parameters (path, start_line, end_line) are still supported for backward compatibility, we recommend using the newer args format for consistency and future compatibility.
...
2. 
Second Priority: Automatic Truncation for Large Text Files
