...
<#
.SYNOPSIS
    Tests les snippets de tests Pester.
.DESCRIPTION
    Ce script teste les snippets de tests Pester en les convertissant en code PowerShell
...
$snippetsFilePath = Join-Path -Path $PSScriptRoot -ChildPath '..\PowerShell-Test-Snippets.json'
...
function Test-Snippet {
    ...
    # Ajouter un en-tête pour les snippets qui ne sont pas des fichiers complets
    if ($Name -ne 'Pester Test File' -and $Name -ne 'Pester Test Function' -and $Name -ne 'Pester Test Module') {
        $code = @(
            "# Fichier de test généré pour le snippet $Name"
            "BeforeAll {"
            "    # Code fictif pour le test"
            "}"
            ""
            "Describe 'Test du snippet $Name' {"
        ) + $code + @(
            "}"
            ""
            "AfterAll {"
    ...
    $tempFile = Join-Path -Path $tempDir -ChildPath "$Name.Tests.ps1"
