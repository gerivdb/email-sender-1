...
.DESCRIPTION
    ...
    Cette version préserve soigneusement les indentations du document original.
...
.NOTES
    ...
    Date de mise à jour: 2025-04-26 - Correction de la préservation des indentations
#>
function Update-ActiveDocumentCheckboxes {
    ...
    try {
        ...
        for ($i = 0; $i -lt $content.Count; $i++) {
            ...
            # Utiliser une regex qui capture l'indentation et le reste de la ligne
            if ($line -match '^(\s*)-\s+\[\s*\](.*)') {
                $indentation = $matches[1]  # Capture l'indentation
                $taskText = $matches[2].Trim()  # Capture le texte de la tâche sans l'indentation
                ...
                if ($taskFound -and $taskComplete) {
                    # Mettre à jour la case à cocher en préservant l'indentation exacte
                    $newLine = "$indentation- [x]$($matches[2])"
                    $content[$i] = $newLine
                    $modified = $true
                    $tasksUpdated++
