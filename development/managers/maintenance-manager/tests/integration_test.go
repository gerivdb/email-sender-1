...
func (suite *FMOUAIntegrationTestSuite) SetupSuite() {
	...
	err = os.MkdirAll(suite.testRepoPath, 0o755)
	require.NoError(suite.T(), err)
...
func (suite *FMOUAIntegrationTestSuite) setupTestRepository() {
	...
	for _, dir := range dirs {
		err := os.MkdirAll(filepath.Join(suite.testRepoPath, dir), 0o755)
		require.NoError(suite.T(), err)
	}
...
func (suite *FMOUAIntegrationTestSuite) TestErrorHandlingAndRecovery() {
	...
	err = os.WriteFile(targetFile, []byte("test content"), 0o644)
	require.NoError(t, err)

	// Make file read-only to trigger permission error
	err = os.Chmod(targetFile, 0o444)
	require.NoError(t, err)
...
func (suite *FMOUAIntegrationTestSuite) createLargeDataset() {
	...
	for i := 0; i < 100; i++ {
		...
		os.MkdirAll(filepath.Dir(jsFile), 0o755)

		os.WriteFile(jsFile, []byte(fmt.Sprintf("console.log('File %d');", i)), 0o644)
		os.WriteFile(cssFile, []byte(fmt.Sprintf(".class%d { color: red; }", i)), 0o644)
		os.WriteFile(htmlFile, []byte(fmt.Sprintf("<html><body>Page %d</body></html>", i)), 0o644)
	}
}
