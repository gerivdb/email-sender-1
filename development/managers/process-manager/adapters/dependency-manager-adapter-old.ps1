...
function Invoke-DependencyManagerCommand {
   ...
   # Construire les arguments pour le binary Go
   $arguments = @()
   ...
   switch ($Command.ToLower()) {
      ...
      "getinfo" {
         # Commande spéciale pour obtenir des informations sur le gestionnaire
         return @{
   ...
   # Exécuter la commande via PowerShell script (qui gère la compilation et l'exécution)
   if ($PSCmdlet.ShouldProcess("Dependency Manager", "Exécuter la commande $Command")) {
      try {
         # Utiliser le script PowerShell qui gère l'exécution du binary
         $scriptArgs = @()
         $scriptArgs += "-Action"
         $scriptArgs += $arguments[0]  # La première partie est l'action
            
         # Ajouter les autres arguments
         for ($i = 1; $i -lt $arguments.Length; $i += 2) {
            if ($i + 1 -lt $arguments.Length) {
               $scriptArgs += $arguments[$i]
               $scriptArgs += $arguments[$i + 1]
            }
            else {
               $scriptArgs += $arguments[$i]
            }
