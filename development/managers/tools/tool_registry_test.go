...
func TestRegisterGlobalTool(t *testing.T) {
	...
	// Register the tool
	err := RegisterGlobalTool("mock-op", mockTool)
	if err != nil {
	...
	registry := GetGlobalRegistry()
	tool, err := registry.GetTool("mock-op")
	if err != nil {
...
// MockTool implements ToolkitOperation for testing
type MockTool struct {
	name string
}

func (m *MockTool) Execute(ctx context.Context, options *OperationOptions) error {
	return nil
}
