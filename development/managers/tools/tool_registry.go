...
func NewToolRegistry() *ToolRegistry {
	return &ToolRegistry{
		tools:     make(map[Operation]ToolkitOperation),
...
func (tr *ToolRegistry) GetTool(op Operation) (ToolkitOperation, error) {
	tr.mutex.RLock()
...
func (tr *ToolRegistry) ListOperations() []Operation {
	tr.mutex.RLock()
	defer tr.mutex.RUnlock()

	operations := make([]Operation, 0, len(tr.tools))
