...
func (sm *storageManagerImpl) initializeQdrant(ctx context.Context) error {
	...
	if err != nil {
		sm.logger.Warn("Qdrant connection test failed", zap.Error(err))
		return nil
	}
	defer resp.Body.Close()
	if resp.StatusCode != http.StatusOK {
		sm.logger.Warn("Qdrant not available", zap.Int("status", resp.StatusCode))
		return nil
	}
...
func (sm *storageManagerImpl) GetDependencyMetadata(ctx context.Context, name string) (*interfaces.DependencyMetadata, error) {
	...
	if err := json.Unmarshal(attributesJSON, &metadata.Attributes); err != nil {
		sm.logger.Warn("Failed to unmarshal attributes", zap.Error(err))
	}
	if err := json.Unmarshal(vulnerabilitiesJSON, &metadata.Vulnerabilities); err != nil {
		sm.logger.Warn("Failed to unmarshal vulnerabilities", zap.Error(err))
	}
...
func (sm *storageManagerImpl) QueryDependencies(ctx context.Context, query *DependencyQuery) ([]*interfaces.DependencyMetadata, error) {
	...
	for rows.Next() {
		...
		if err := json.Unmarshal(tagsJSON, &metadata.Tags); err != nil {
			sm.logger.Warn("Failed to unmarshal tags for "+metadata.Name, zap.Error(err))
		}
		if err := json.Unmarshal(attributesJSON, &metadata.Attributes); err != nil {
			sm.logger.Warn("Failed to unmarshal attributes for "+metadata.Name, zap.Error(err))
		}
		if err := json.Unmarshal(vulnerabilitiesJSON, &metadata.Vulnerabilities); err != nil {
			sm.logger.Warn("Failed to unmarshal vulnerabilities for "+metadata.Name, zap.Error(err))
		}
...
func (sm *storageManagerImpl) HealthCheck(ctx context.Context) error {
	...
	if sm.qdrantURL != "" {
		...
		if err == nil {
			...
			if errHealth != nil {
				sm.logger.Warn("Qdrant health check failed", zap.Error(errHealth))
			} else {
				resp.Body.Close()
				if resp.StatusCode != http.StatusOK {
