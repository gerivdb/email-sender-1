...
func TestGitWorkflowManagerCreation(t *testing.T) {
	...
	manager, err := main.NewGitWorkflowManager(errorManager, configManager, storageManager, config)
	if err != nil {
...
func TestGitWorkflowManagerWithMissingDependencies(t *testing.T) {
	...
	_, err := main.NewGitWorkflowManager(nil, &MockConfigManager{}, &MockStorageManager{}, config)
	if err == nil {
		t.Error("Expected error when errorManager is nil")
	}
	
	// Test with nil configManager
	_, err = main.NewGitWorkflowManager(&MockErrorManager{}, nil, &MockStorageManager{}, config)
	if err == nil {
		t.Error("Expected error when configManager is nil")
	}
	
	// Test with nil storageManager
	_, err = main.NewGitWorkflowManager(&MockErrorManager{}, &MockConfigManager{}, nil, config)
	if err == nil {
...
func TestFactoryCreation(t *testing.T) {
	factory := main.NewGitWorkflowManagerFactory()
	if factory == nil {
