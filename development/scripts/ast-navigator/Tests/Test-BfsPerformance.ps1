# Script de test de performance pour les fonctions de parcours en largeur (BFS) de l'AST

# Importer le module AST Navigator
$modulePath = Join-Path -Path $PSScriptRoot -ChildPath "..\AstNavigator.psm1"
Import-Module $modulePath -Force

# Cr̩er un script PowerShell complexe pour les tests de performance
$complexScript = @'
...
$results += Test-Performance -FunctionName "Invoke-AstTraversalBFSAdvanced" -Ast $ast -NodeType "FunctionDefinition"
...
$results += Test-Performance -FunctionName "Invoke-AstTraversalBFSAdvanced" -Ast $ast -NodeType "VariableExpressionAst"
...
$results += Test-Performance -FunctionName "Invoke-AstTraversalBFSAdvanced" -Ast $ast -Predicate $customPredicate
...
Write-Host "`nAmeliorations de performance par rapport a Invoke-AstTraversalBFS:" -ForegroundColor Green
$baselineResults = $results | Where-Object { $_.FunctionName -eq "Invoke-AstTraversalBFS" }
$baselineAvgTime = ($baselineResults | Measure-Object -Property ElapsedTime -Average).Average

$results | Where-Object { $_.FunctionName -ne "Invoke-AstTraversalBFS" } | Group-Object -Property FunctionName | ForEach-Object {
    $functionName = $_.Name
    $avgTime = ($_.Group | Measure-Object -Property ElapsedTime -Average).Average
