...
<#
.SYNOPSIS
    Tests pour le module TestStandardModule.
.DESCRIPTION
    Ce script contient des tests Pester pour le module TestStandardModule.
.EXAMPLE
    Invoke-Pester -Path ".\TestStandardModule.Tests.ps1"
.NOTES
    Version: 1.0.0
    Auteur: Test User
...
Describe "TestStandardModule Module Tests" {
    Context "Module Loading" {
        ...
        It "Should have exported functions" {
            $ExportedFunctions = Get-Command -Module TestStandardModule -CommandType Function
...
AfterAll {
    ...
    Remove-Module -Name TestStandardModule -ErrorAction SilentlyContinue
}
