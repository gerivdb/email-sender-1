...
<#
.SYNOPSIS
    Tests pour le module HygenAdvancedModule.
.DESCRIPTION
    Ce script contient des tests Pester pour le module HygenAdvancedModule.
.EXAMPLE
    Invoke-Pester -Path ".\HygenAdvancedModule.Tests.ps1"
.NOTES
    Version: 1.0.0
...
Describe "HygenAdvancedModule Module Tests" {
    Context "Module Loading" {
        ...
        It "Should have exported functions" {
            $ExportedFunctions = Get-Command -Module HygenAdvancedModule -CommandType Function
...
AfterAll {
    ...
    Remove-Module -Name HygenAdvancedModule -ErrorAction SilentlyContinue
}
