...
"@
    ...
    $selectedFunction = $correctFunctions | Get-Random
    ...
    for ($i = 0; $i -lt $ErrorCount; $i++) {
        ...
        $error = $errorTemplates[$errorType] | Get-Random

        # Appliquer l'erreur au contenu du script
        $matches = [regex]::Matches($scriptContent, $error.Pattern)
        if ($matches.Count -gt 0) {
            $randomMatch = $matches | Get-Random
            $scriptContent = $scriptContent.Substring(0, $randomMatch.Index) +
            $scriptContent.Substring($randomMatch.Index).Replace($randomMatch.Value, $error.Replacement, 1)

            # Ajouter un commentaire pour identifier l'erreur
            $scriptContent = $scriptContent.Replace(
                $error.Replacement,
                "$($error.Replacement) # Erreur inject̩e: $($error.Description)"
            )
        }
    }
