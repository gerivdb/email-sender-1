...
function New-TestTemplates {
    ...
    # CrÃƒÂ©er un template de base de test
...
<body>
    ...
    <p>PÃƒÂ©riode: {{report_period_start}} - {{report_period_end}}</p>
    <p>GÃƒÂ©nÃƒÂ©rÃƒÂ© le: {{report_generated_at}}</p>
    
    <div class="toc">
        <h2>Table des matiÃƒÂ¨res</h2>
        <ul>
            {{toc_items}}
        </ul>
    </div>
...
"@
    ...
    # CrÃƒÂ©er un template de sections de test
    $SectionTemplatesPath = Join-Path -Path $TemplatesDir -ChildPath "test_sectiondevelopment/templates.html"
...
<!-- Template pour un ÃƒÂ©lÃƒÂ©ment de la table des matiÃƒÂ¨res -->
    ...
    return @{
        ...
        sections = @(
            ...
            @{
                ...
                chart_description = "Ãƒâ€°volution de l'utilisation CPU sur la pÃƒÂ©riode."
            },
            @{
                id = "anomalies"
                title = "Anomalies dÃƒÂ©tectÃƒÂ©es"
                type = "anomalies"
                anomalies = @(
                    @{
                        metric = "CPU"
...
Describe "HTML Exporter Module Tests" {
    ...
    Context "Export-ReportToHtml function" {
        It "Should export a report to HTML correctly" {
            ...
            $Content | Should -BeLike "*RÃƒÂ©sumÃƒÂ©*"
            $Content | Should -BeLike "*MÃƒÂ©triques clÃƒÂ©s*"
            $Content | Should -BeLike "*Utilisation CPU*"
            $Content | Should -BeLike "*Anomalies dÃƒÂ©tectÃƒÂ©es*"
        }
    }
}
