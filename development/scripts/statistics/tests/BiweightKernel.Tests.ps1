# Tests pour les fonctions du noyau biweight (quartic) dans l'estimation de densité par noyau
# Run with Pester: Invoke-Pester -Path ".\BiweightKernel.Tests.ps1"

BeforeAll {
    # Charger le module du noyau biweight
    $modulePath = Join-Path -Path $PSScriptRoot -ChildPath "..\BiweightKernel.ps1"
    if (Test-Path $modulePath) {
        . $modulePath
    } else {
        throw "Le module BiweightKernel.ps1 n'a pas été trouvé dans le répertoire parent."
    }
...
Describe "Tests pour les fonctions du noyau biweight" {
    Context "Tests de base pour Get-BiweightKernel" {
        It "Devrait calculer la valeur correcte pour u = 0" {
            $result = Get-BiweightKernel -U 0
            $expected = 15.0 / 16.0
            $result | Should -BeApproximately $expected -Epsilon 0.0001
        }
        
        It "Devrait calculer la valeur correcte pour u = 0.5" {
            $result = Get-BiweightKernel -U 0.5
            $expected = (15.0 / 16.0) * [Math]::Pow(1 - 0.25, 2)
            $result | Should -BeApproximately $expected -Epsilon 0.0001
        }
        
        It "Devrait retourner 0 pour u > 1" {
            $result = Get-BiweightKernel -U 1.5
            $result | Should -Be 0
        }
        
        It "Devrait être symétrique" {
            $result1 = Get-BiweightKernel -U 0.5
            $result2 = Get-BiweightKernel -U -0.5
            $result1 | Should -BeApproximately $result2 -Epsilon 0.0001
        }
    }
    
    Context "Tests de base pour Get-BiweightKernelDensity" {
        ...
        It "Devrait calculer la densité correctement" {
            $result = Get-BiweightKernelDensity -X 50 -Data $script:testData1 -Bandwidth 5
            $result | Should -BeGreaterThan 0
        }
        
        It "Devrait calculer la largeur de bande optimale si non spécifiée" {
            $result = Get-BiweightKernelDensity -X 50 -Data $script:testData1
            $result | Should -BeGreaterThan 0
        }
        
        It "Devrait lever une exception si les données contiennent moins de 2 points" {
            { Get-BiweightKernelDensity -X 50 -Data @(1) } | Should -Throw "Les données doivent contenir au moins 2 points pour l'estimation de densité par noyau."
        }
    }
    
    Context "Tests de base pour Get-OptimizedBiweightKernelDensity" {
        # Générer des données de test en dehors du bloc BeforeAll
        # pour éviter les avertissements PSScriptAnalyzer sur les variables non utilisées
        $script:testData2 = Get-NormalSample -NumPoints 100 -Mean 50 -StdDev 10
        
        It "Devrait calculer la densité correctement" {
            $result = Get-OptimizedBiweightKernelDensity -X 50 -Data $script:testData2 -Bandwidth 5
            $result | Should -BeGreaterThan 0
        }
        
        It "Devrait calculer la largeur de bande optimale si non spécifiée" {
            $result = Get-OptimizedBiweightKernelDensity -X 50 -Data $script:testData2
            $result | Should -BeGreaterThan 0
        }
        
        It "Devrait lever une exception si les données contiennent moins de 2 points" {
            { Get-OptimizedBiweightKernelDensity -X 50 -Data @(1) } | Should -Throw "Les données doivent contenir au moins 2 points pour l'estimation de densité par noyau."
        }
        
        It "Devrait donner des résultats cohérents avec la fonction de base" {
            $resultBase = Get-BiweightKernelDensity -X 50 -Data $script:testData2 -Bandwidth 5
            $resultOptimized = Get-OptimizedBiweightKernelDensity -X 50 -Data $script:testData2 -Bandwidth 5
            
            $resultOptimized | Should -BeApproximately $resultBase -Epsilon 0.0001
        }
    }
    
    Context "Tests de base pour Get-OptimizedBiweightKernelDensityMultiplePoints" {
        # Générer des données de test en dehors du bloc BeforeAll
        # pour éviter les avertissements PSScriptAnalyzer sur les variables non utilisées
        ...
        It "Devrait calculer la densité correctement pour plusieurs points" {
            $result = Get-OptimizedBiweightKernelDensityMultiplePoints -EvaluationPoints $script:evalPoints1 -Data $script:testData3 -Bandwidth 5
        ...
        It "Devrait calculer la largeur de bande optimale si non spécifiée" {
            $result = Get-OptimizedBiweightKernelDensityMultiplePoints -EvaluationPoints $script:evalPoints1 -Data $script:testData3
            $result | Should -Not -BeNullOrEmpty
            $result.Count | Should -Be $script:evalPoints1.Count
            $result[0].Density | Should -BeGreaterThan 0
        }
        ...
        It "Devrait lever une exception si les données contiennent moins de 2 points" {
            { Get-OptimizedBiweightKernelDensityMultiplePoints -EvaluationPoints $script:evalPoints1 -Data @(1) } | Should -Throw "Les données doivent contenir au moins 2 points pour l'estimation de densité par noyau."
        }
        
        It "Devrait donner des résultats cohérents avec la fonction de base" {
            $resultBase = $script:evalPoints1 | ForEach-Object { Get-BiweightKernelDensity -X $_ -Data $script:testData3 -Bandwidth 5 }
            $resultOptimized = Get-OptimizedBiweightKernelDensityMultiplePoints -EvaluationPoints $script:evalPoints1 -Data $script:testData3 -Bandwidth 5
    ...
    Context "Tests de base pour Get-BiweightKernelDensityND" {
        ...
        It "Devrait calculer la densité correctement pour des données bidimensionnelles" {
            $result = Get-BiweightKernelDensityND -Point $script:point2D -Data $script:testData2D
            $result | Should -BeGreaterThan 0
        }
        ...
        It "Devrait lever une exception si les données contiennent moins de 2 points" {
            { Get-BiweightKernelDensityND -Point $script:point2D -Data @($script:testData2D[0]) } | Should -Throw "Les données doivent contenir au moins 2 points pour l'estimation de densité par noyau."
        }
        
        It "Devrait fonctionner avec une largeur de bande unique" {
            $result = Get-BiweightKernelDensityND -Point $script:point2D -Data $script:testData2D -Bandwidth 10
            $result | Should -BeGreaterThan 0
        }
        ...
        It "Devrait fonctionner avec une largeur de bande par dimension" {
            ...
            $result = Get-BiweightKernelDensityND -Point $script:point2D -Data $script:testData2D -Bandwidth $bandwidth
            $result | Should -BeGreaterThan 0
        }
        ...
        It "Devrait lever une exception si la largeur de bande ne contient pas les dimensions spécifiées" {
            ...
            { Get-BiweightKernelDensityND -Point $script:point2D -Data $script:testData2D -Bandwidth $bandwidth } | Should -Throw "La largeur de bande ne contient pas la dimension spécifiée: Y"
        }
        ...
        It "Devrait lever une exception si le point d'évaluation ne contient pas les dimensions spécifiées" {
            ...
            { Get-BiweightKernelDensityND -Point $invalidPoint -Data $script:testData2D } | Should -Throw "Le point d'évaluation ne contient pas la dimension spécifiée: Y"
        }
    }
    ...
    Context "Tests avec des données tridimensionnelles" {
        ...
        It "Devrait calculer la densité correctement pour des données tridimensionnelles" {
            $result = Get-BiweightKernelDensityND -Point $script:point3D -Data $script:testData3D
            $result | Should -BeGreaterThan 0
        }
        
        It "Devrait fonctionner avec un sous-ensemble de dimensions" {
            $result = Get-BiweightKernelDensityND -Point $script:point3D -Data $script:testData3D -Dimensions @("X", "Y")
            $result | Should -BeGreaterThan 0
        }
    }
    ...
    Context "Tests de performance" {
        # Générer des données de test en dehors du bloc BeforeAll
        # pour éviter les avertissements PSScriptAnalyzer sur les variables non utilisées
        $script:testData4 = Get-NormalSample -NumPoints 1000 -Mean 50 -StdDev 10
        ...
        It "La fonction optimisée devrait être plus rapide que la fonction de base" {
            ...
            $startTime = Get-Date
            $script:evalPoints2 | ForEach-Object { Get-BiweightKernelDensity -X $_ -Data $script:testData4 -Bandwidth 5 }
            $endTime = Get-Date
            $baseTime = ($endTime - $startTime).TotalMilliseconds
            
            # Mesurer le temps d'exécution de la fonction optimisée
            $startTime = Get-Date
            $script:evalPoints2 | ForEach-Object { Get-OptimizedBiweightKernelDensity -X $_ -Data $script:testData4 -Bandwidth 5 }
            $endTime = Get-Date
        ...
        It "La fonction optimisée pour plusieurs points devrait être plus rapide que la fonction optimisée pour un seul point" {
            ...
            $startTime = Get-Date
            Get-OptimizedBiweightKernelDensityMultiplePoints -EvaluationPoints $script:evalPoints2 -Data $script:testData4 -Bandwidth 5
            $endTime = Get-Date
