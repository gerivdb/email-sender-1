...
# Test 1: Valeurs du noyau triangulaire
Write-Host "`n=== Test 1: Valeurs du noyau triangulaire ===" -ForegroundColor Magenta
$testPoints = @(0, 0.25, 0.5, 0.75, 1.0, 1.25, 1.5)
Write-Host "Valeurs du noyau triangulaire pour différents points:" -ForegroundColor White
Write-Host "| Point | Valeur du noyau |" -ForegroundColor White
Write-Host "|-------|----------------|" -ForegroundColor White
foreach ($point in $testPoints) {
    $kernelValue = Get-TriangularKernel -U $point
    Write-Host "| $point | $([Math]::Round($kernelValue, 6)) |" -ForegroundColor Green
}

# Vérifier que la valeur au centre (u=0) est correcte
$centerValue = Get-TriangularKernel -U 0
$expectedCenterValue = 1.0
...
# Test 2: Symétrie du noyau triangulaire
Write-Host "`n=== Test 2: Symétrie du noyau triangulaire ===" -ForegroundColor Magenta
$symmetryTestPoints = @(0.25, 0.5, 0.75, 1.0)
$symmetryCorrect = $true
Write-Host "Vérification de la symétrie du noyau triangulaire:" -ForegroundColor White
Write-Host "| Point | Valeur à +u | Valeur à -u | Différence |" -ForegroundColor White
Write-Host "|-------|------------|------------|------------|" -ForegroundColor White
foreach ($point in $symmetryTestPoints) {
    $positiveValue = Get-TriangularKernel -U $point
    $negativeValue = Get-TriangularKernel -U (-$point)
...
Write-Host "`nSymétrie du noyau triangulaire: $(if ($symmetryCorrect) { "Correcte" } else { "Incorrecte" })" -ForegroundColor $(if ($symmetryCorrect) { "Green" } else { "Red" })

# Test 3: Support compact du noyau triangulaire
Write-Host "`n=== Test 3: Support compact du noyau triangulaire ===" -ForegroundColor Magenta
$compactSupportTestPoints = @(0.9, 0.99, 1.0, 1.001, 1.5, 2.0)
$compactSupportCorrect = $true
Write-Host "Vérification du support compact du noyau triangulaire:" -ForegroundColor White
Write-Host "| Point | Valeur du noyau | Attendu |" -ForegroundColor White
Write-Host "|-------|----------------|---------|" -ForegroundColor White
foreach ($point in $compactSupportTestPoints) {
    $kernelValue = Get-TriangularKernel -U $point
    $expected = if ([Math]::Abs($point) -lt 1) { "Positif" } elseif ([Math]::Abs($point) -eq 1) { "0" } else { "0" }
...
Write-Host "`nSupport compact du noyau triangulaire: $(if ($compactSupportCorrect) { "Correct" } else { "Incorrect" })" -ForegroundColor $(if ($compactSupportCorrect) { "Green" } else { "Red" })

# Test 4: Intégration du noyau triangulaire
Write-Host "`n=== Test 4: Intégration du noyau triangulaire ===" -ForegroundColor Magenta
# L'intégrale du noyau triangulaire sur l'intervalle [-1, 1] devrait être égale à 1
$integrationPoints = -1..1  # Points de -1 à 1 avec un pas de 1
$integrationValues = $integrationPoints | ForEach-Object { Get-TriangularKernel -U $_ }
...
Write-Host "Intégrale approximative du noyau triangulaire sur [-1, 1]: $([Math]::Round($integrationSum, 4))" -ForegroundColor White
...
# Test 5: Estimation de densité par noyau triangulaire
Write-Host "`n=== Test 5: Estimation de densité par noyau triangulaire ===" -ForegroundColor Magenta
# Calculer la densité à différents points
$densityPoints = @(70, 85, 100, 115, 130)
$bandwidth = 0  # Utiliser la largeur de bande optimale
Write-Host "Estimation de densité par noyau triangulaire pour une distribution normale (μ=100, σ=15):" -ForegroundColor White
Write-Host "| Point | Densité estimée |" -ForegroundColor White
Write-Host "|-------|----------------|" -ForegroundColor White
foreach ($point in $densityPoints) {
    $density = Get-TriangularKernelDensity -X $point -Data $normalData -Bandwidth $bandwidth
...
$densities = $densityPoints | ForEach-Object { 
    [PSCustomObject]@{
        Point = $_
        Density = Get-TriangularKernelDensity -X $_ -Data $normalData -Bandwidth $bandwidth
    }
}
...
# Test 6: Comparaison avec les autres noyaux
Write-Host "`n=== Test 6: Comparaison avec les autres noyaux ===" -ForegroundColor Magenta
# Calculer la densité avec les trois noyaux
$comparisonPoint = 100  # Point central
$triangularDensity = Get-TriangularKernelDensity -X $comparisonPoint -Data $normalData -Bandwidth $bandwidth
...
Write-Host "|-------|----------------|" -ForegroundColor White
Write-Host "| Triangulaire | $([Math]::Round($triangularDensity, 6)) |" -ForegroundColor Green
...
Write-Host "`nRatio Triangulaire/Gaussien: $([Math]::Round($triangularDensity / $gaussianDensity, 4))" -ForegroundColor White
Write-Host "Ratio Triangulaire/Epanechnikov: $([Math]::Round($triangularDensity / $epanechnikovDensity, 4))" -ForegroundColor White
