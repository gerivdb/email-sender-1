...
.DESCRIPTION
    ...
    ont été ajoutés à la racine du dossier manager et les organise automatiquement.
.PARAMETER Force
...
# Vérifier si des fichiers PowerShell ont été ajoutés à la racine du dossier manager
MANAGER_DIR="development/scripts/manager"
ADDED_PS_FILES=$(git diff --cached --name-only --diff-filter=A | grep -E "^$MANAGER_DIR/[^/]+\.(ps1|psm1|psd1)$")

if [ -n "$ADDED_PS_FILES" ]; then
    echo "Des fichiers PowerShell ont été ajoutés à la racine du dossier manager:"
    echo "$ADDED_PS_FILES"
...
Write-Log "Le hook organisera automatiquement les scripts PowerShell ajoutés à la racine des dossiers manager et maintenance." -Level "INFO"
