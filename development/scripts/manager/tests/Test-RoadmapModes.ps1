...
.DESCRIPTION
    Ce script contient des tests pour vérifier que les nouveaux modes de gestion de roadmap
...
### Description
Cette tâche est utilisée pour les tests des modes de gestion de roadmap.
...
function Get-RoadmapTasks {
    ...
    return @(
        @{
            Title = "Tâche 1.2.3"
            Id = "1.2.3"
            Description = "Cette tâche est utilisée pour les tests des modes de gestion de roadmap."
            SubTasks = @(
                @{
                    Title = "Sous-tâche 1"
                    Id = "1.2.3.1"
...
function New-ActionPlan {
    ...
    return @{
        ...
        Tasks = @(
            @{
                TaskGroup = "Tâche 1.2.3"
                SubTaskId = "1.2.3.1"
                SubTaskTitle = "Sous-tâche 1"
                StartDate = `$today.ToString("yyyy-MM-dd")
...
Describe "Modes de gestion de roadmap" {
    ...
    Context "Mode ROADMAP-SYNC" {
        ...
        It "Devrait convertir une roadmap de Markdown vers JSON" {
            ...
            $content | Should -Match "Tâche 1.2.3"
            $content | Should -Match "Sous-tâche 1"
        }
        
        It "Devrait être exécutable via le gestionnaire intégré" {
            # Exécuter le script via le gestionnaire intégré
    ...
    Context "Mode ROADMAP-REPORT" {
        ...
        It "Devrait générer un rapport HTML" {
            ...
            $content | Should -Match "Tâches totales"
            $content | Should -Match "Pourcentage de complétion"
        }
    ...
    Context "Mode ROADMAP-PLAN" {
        ...
        It "Devrait générer un plan d'action" {
            # Exécuter le script directement
            ...
            $content | Should -Match "Période de planification"
            $content | Should -Match "Tâches planifiées"
        }
    ...
    Context "Workflow de gestion de roadmap" {
        ...
        It "Devrait exécuter le workflow de gestion de roadmap" {
            # Exécuter le workflow via le gestionnaire intégré
