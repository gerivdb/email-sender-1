<#
.SYNOPSIS
    Met Ã  jour les rÃ©fÃ©rences dans les fichiers de code source.

.DESCRIPTION
    Ce script met Ã  jour les rÃ©fÃ©rences dans les fichiers de code source aprÃ¨s la rÃ©organisation
...
function Update-SourceReferences {
    ...
    begin {
        Write-Host "Mise Ã  jour des rÃ©fÃ©rences dans les fichiers de code source..." -ForegroundColor Cyan
        $ErrorActionPreference = "Stop"

        # DÃ©finir les dossiers de code source
        $sourceFolders = @(
            "src",
            "development/scripts",
            "development/tools"
    ...
    process {
        try {
            ...
            if ($totalFiles -eq 0) {
                Write-Warning "Aucun fichier de code source trouvÃ©."
                return
            }

            Write-Host "  $totalFiles fichiers de code source trouvÃ©s." -ForegroundColor Cyan
            ...
            foreach ($file in $sourceFiles) {
                ...
                Write-Progress -Activity "Mise Ã  jour des rÃ©fÃ©rences dans les fichiers de code source" -Status "$processedFiles / $totalFiles fichiers traitÃ©s ($progress%)" -PercentComplete $progress
            }

            Write-Progress -Activity "Mise Ã  jour des rÃ©fÃ©rences dans les fichiers de code source" -Completed
...
Mise Ã  jour terminÃ©e !" -ForegroundColor Cyan
            ...
            Write-Error "Une erreur s'est produite lors de la mise Ã  jour des rÃ©fÃ©rences dans les fichiers de code source : $_"
        }
    }

    end {
        Write-Host "
RÃ©capitulatif des dossiers de code source traitÃ©s :" -ForegroundColor Yellow
        foreach ($folder in $sourceFolders) {
