<#
.SYNOPSIS
    Met Ã  jour les rÃ©fÃ©rences aprÃ¨s la rÃ©organisation de la structure de la roadmap.

.DESCRIPTION
    Ce script met Ã  jour les rÃ©fÃ©rences dans les fichiers du projet aprÃ¨s la rÃ©organisation
    de la structure des dossiers de la roadmap.

.EXAMPLE
    .\update-roadmap-references.ps1
...
# Fonction principale
function Update-RoadmapReferences {
    [CmdletBinding(SupportsShouldProcess=$true)]
    param()
    
    begin {
        Write-Host "Mise Ã  jour des rÃ©fÃ©rences aprÃ¨s la rÃ©organisation de la structure de la roadmap..." -ForegroundColor Cyan
        $ErrorActionPreference = "Stop"
        
        # DÃ©finir les mappages de chemins
        $pathMappings = @(
            @{
                OldPath = "development/roadmap/scripts"
                NewPath = "development/roadmap/scripts"
                Description = "DÃ©placement des scripts de roadmap"
            },
            @{
                OldPath = "development/roadmap/scripts-parser"
                NewPath = "development/roadmap/parser"
                Description = "DÃ©placement du parser de roadmap"
            }
        )
    }
