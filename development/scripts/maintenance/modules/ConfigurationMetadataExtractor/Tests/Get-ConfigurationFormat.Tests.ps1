...
Describe 'Get-ConfigurationFormat' {
    Context 'DÃ©tection du format Ã  partir du contenu' {
        ...
        It 'DÃ©tecte correctement le format PSD1' {
    ...
    Context 'DÃ©tection du format Ã  partir du chemin' {
        BeforeAll {
        ...
        It 'DÃ©tecte correctement le format JSON Ã  partir du chemin' {
            $result = Get-ConfigurationFormat -Path $jsonPath
            $result | Should -Be 'JSON'
        }
        
        It 'DÃ©tecte correctement le format YAML Ã  partir du chemin' {
            $result = Get-ConfigurationFormat -Path $yamlPath
            $result | Should -Be 'YAML'
        }
        
        It 'DÃ©tecte correctement le format XML Ã  partir du chemin' {
            $result = Get-ConfigurationFormat -Path $xmlPath
            $result | Should -Be 'XML'
        }
        
        It 'DÃ©tecte correctement le format INI Ã  partir du chemin' {
            $result = Get-ConfigurationFormat -Path $iniPath
            $result | Should -Be 'INI'
        }
        
        It 'DÃ©tecte correctement le format PSD1 Ã  partir du chemin' {
            $result = Get-ConfigurationFormat -Path $psd1Path
            $result | Should -Be 'PSD1'
        }
        
        It 'GÃ©nÃ¨re une erreur pour un chemin inexistant' {
