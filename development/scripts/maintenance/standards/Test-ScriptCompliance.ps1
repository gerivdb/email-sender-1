...
.EXAMPLE
    .\Test-ScriptCompliance.ps1 -Path "D:\scripts" -ScriptType PowerShell
    ...
    switch ($ScriptType) {
        "PowerShell" {
            ...
            if (-not ($Content -match "<#[\s\S]*?#>")) {
                ...
                $HeaderBlock = $Content -match "<#[\s\S]*?#>" | Out-Null
        ...
        "Python" {
            ...
            if (-not ($Content -match '"""[\s\S]*?"""' -or $Content -match "'''[\s\S]*?'''")) {
                ...
                # V̩rifier les ̩l̩ments requis dans l'en-t̻te
                $HeaderBlock = $Content -match '"""[\s\S]*?"""' -or $Content -match "'''[\s\S]*?'''" | Out-Null
                $HeaderContent = $Matches[0]
...
function Test-CodeStyle {
    ...
    switch ($ScriptType) {
        ...
        "Shell" {
            ...
            $VarReferences = [regex]::Matches($Content, "\$([A-Za-z0-9_]+)")
            foreach ($Reference in $VarReferences) {
                if (-not ($Reference.Value -match "\$\{")) {
                    $Issues += [PSCustomObject]@{
                        Rule = "VarReference"
                        Description = "R̩f̩rence de variable non prot̩g̩e: '$($Reference.Value)'. Utilisez plut̫t '`${$($Reference.Groups[1].Value)}'"
                        Severity = "Low"
                    }
                }
            }
