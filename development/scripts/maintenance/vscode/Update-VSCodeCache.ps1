# Script pour mettre ÃƒÂ  jour les chemins dans le fichier mtime-cache.json de VS Code
# Ce script remplace les anciens chemins par les nouveaux chemins dans le fichier de cache VS Code

Write-Host "=== Mise ÃƒÂ  jour des chemins dans le fichier mtime-cache.json de VS Code ===" -ForegroundColor Cyan
...
$oldPathVariants = @(
    ...
    "D:\\DO\\WEB\\N8N tests\\scripts json ÃƒÂ  tester\\EMAIL SENDER 1",
    "D:/DO/WEB/N8N development/testing/tests/scripts json ÃƒÂ  tester/EMAIL SENDER 1",
    "D:\DO\WEB\N8N tests\scripts json ÃƒÂ  tester\EMAIL SENDER 1"
)
...
if (-not (Test-Path $mtimeCachePath)) {
    Write-Host "Ã¢ÂÅ’ Le fichier mtime-cache.json n'existe pas ÃƒÂ  l'emplacement spÃƒÂ©cifiÃƒÂ©." -ForegroundColor Red
    exit 1
}
...
try {
    ...
    Write-Host "Ã¢Å“â€¦ Fichier mtime-cache.json lu avec succÃƒÂ¨s." -ForegroundColor Green
}
catch {
    Write-Host "Ã¢ÂÅ’ Erreur lors de la lecture du fichier mtime-cache.json : $_" -ForegroundColor Red
    exit 1
}

# CrÃƒÂ©er une copie de sauvegarde du fichier
$backupPath = "$mtimeCachePath.backup"
try {
    Copy-Item -Path $mtimeCachePath -Destination $backupPath -Force -ErrorAction Stop
    Write-Host "Ã¢Å“â€¦ Copie de sauvegarde crÃƒÂ©ÃƒÂ©e : $backupPath" -ForegroundColor Green
}
catch {
    Write-Host "Ã¢ÂÅ’ Erreur lors de la crÃƒÂ©ation de la copie de sauvegarde : $_" -ForegroundColor Red
    exit 1
}
...
foreach ($variant in $oldPathVariants) {
    # Ãƒâ€°chapper les caractÃƒÂ¨res spÃƒÂ©ciaux pour la regex
...
# Enregistrer le fichier modifiÃƒÂ©
if ($modified) {
    try {
        Set-Content -Path $mtimeCachePath -Value $content -NoNewline -Encoding UTF8 -ErrorAction Stop
        Write-Host "Ã¢Å“â€¦ Fichier mtime-cache.json mis ÃƒÂ  jour avec succÃƒÂ¨s." -ForegroundColor Green
    }
    catch {
        Write-Host "Ã¢ÂÅ’ Erreur lors de l'enregistrement du fichier mtime-cache.json : $_" -ForegroundColor Red
    ...
    Write-Host "Ã¢â€žÂ¹Ã¯Â¸Â Aucune modification nÃƒÂ©cessaire dans le fichier mtime-cache.json." -ForegroundColor Blue
}
