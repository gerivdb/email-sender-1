<#
.SYNOPSIS
    Script pour ex̩cuter les tests des scripts de traitement avec Jobs PowerShell.
.DESCRIPTION
    Ce script ex̩cute les tests des scripts de traitement avec Jobs PowerShell.
.EXAMPLE
    .\Run-JobsProcessingTests.ps1
    Ex̩cute les tests des scripts de traitement avec Jobs PowerShell.
#>
...
# D̩finir le chemin du test des scripts de traitement avec Jobs PowerShell
$testFile = Join-Path -Path $PSScriptRoot -ChildPath "Tests\JobsProcessing.Tests.ps1"

# Afficher les tests trouv̩s
Write-Host "Test des scripts de traitement avec Jobs PowerShell trouv̩ :" -ForegroundColor Cyan
Write-Host "  JobsProcessing.Tests.ps1" -ForegroundColor Yellow

# Ex̩cuter les tests
Write-Host "`nEx̩cution des tests des scripts de traitement avec Jobs PowerShell..." -ForegroundColor Cyan
