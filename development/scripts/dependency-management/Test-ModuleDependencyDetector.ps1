...
.DESCRIPTION
    Ce script teste les fonctionnalités du module ModuleDependencyDetector
    en analysant différents types d'instructions Import-Module.
...
# Créer un script de test avec différentes formes d'Import-Module et using module
...
# Import-Module avec paramètre nommé
...
# Import-Module avec paramètres supplémentaires
...
Write-Host "Nombre d'instructions Import-Module trouvées: $($result1.Count)" -ForegroundColor Yellow
...
Write-Host "Nombre d'instructions Import-Module trouvées: $($result2.Count)" -ForegroundColor Yellow
$result2 | ForEach-Object { Format-ModuleInfo -ModuleInfo $_ } | ForEach-Object { Write-Host "  $_" -ForegroundColor Green }

# Test 3: Résoudre les chemins des modules
Write-Host "`nTest 3: Résoudre les chemins des modules" -ForegroundColor Cyan
$result3 = Find-ImportModuleInstruction -FilePath $testScriptPath -ResolveModulePaths
Write-Host "Nombre d'instructions Import-Module trouvées: $($result3.Count)" -ForegroundColor Yellow
