...
if (Test-Path -Path $databasePath) {
    Write-Host "  âœ“ Base de donnÃ©es crÃ©Ã©e avec succÃ¨s: $databasePath" -ForegroundColor Green
    ...
    if ($content -match "CmdletToModuleMapping" -and $content -match "TypeToModuleMapping" -and $content -match "VariableToModuleMapping") {
        Write-Host "  âœ“ La base de donnÃ©es contient les sections attendues" -ForegroundColor Green
    } else {
        Write-Host "  âœ— La base de donnÃ©es ne contient pas les sections attendues" -ForegroundColor Red
    }
} else {
    Write-Host "  âœ— Ã‰chec de la crÃ©ation de la base de donnÃ©es" -ForegroundColor Red
}
...
if (Test-Path -Path $updatedDatabasePath) {
    Write-Host "  âœ“ Base de donnÃ©es mise Ã  jour avec succÃ¨s: $updatedDatabasePath" -ForegroundColor Green
    ...
    if ($content -match "Microsoft.PowerShell.Security") {
        Write-Host "  âœ“ La base de donnÃ©es mise Ã  jour contient le module ajoutÃ©" -ForegroundColor Green
    } else {
        Write-Host "  âœ— La base de donnÃ©es mise Ã  jour ne contient pas le module ajoutÃ©" -ForegroundColor Red
    }
} else {
    Write-Host "  âœ— Ã‰chec de la mise Ã  jour de la base de donnÃ©es" -ForegroundColor Red
}
...
if ($importedDatabase) {
    Write-Host "  âœ“ Base de donnÃ©es importÃ©e avec succÃ¨s" -ForegroundColor Green
    ...
    if ($importedDatabase.CmdletToModuleMapping -and $importedDatabase.TypeToModuleMapping -and $importedDatabase.VariableToModuleMapping) {
        Write-Host "  âœ“ La base de donnÃ©es importÃ©e contient les sections attendues" -ForegroundColor Green
    } else {
        Write-Host "  âœ— La base de donnÃ©es importÃ©e ne contient pas les sections attendues" -ForegroundColor Red
    }
} else {
    Write-Host "  âœ— Ã‰chec de l'importation de la base de donnÃ©es" -ForegroundColor Red
}
...
if ($newCmdletCount -gt $originalCmdletCount -or $newTypeCount -gt $originalTypeCount -or $newVariableCount -gt $originalVariableCount) {
    Write-Host "  âœ“ Les mappings globaux ont Ã©tÃ© mis Ã  jour avec succÃ¨s" -ForegroundColor Green
} else {
    Write-Host "  âœ— Les mappings globaux n'ont pas Ã©tÃ© mis Ã  jour correctement" -ForegroundColor Red
}
...
if ($allModulesDatabase) {
    Write-Host "  âœ“ Base de donnÃ©es crÃ©Ã©e avec succÃ¨s sans spÃ©cifier de modules" -ForegroundColor Green
    
    # VÃ©rifier le contenu de la base de donnÃ©es
    if ($allModulesDatabase.CmdletToModuleMapping.Count -gt 0) {
        Write-Host "  âœ“ La base de donnÃ©es contient des mappings de cmdlets ($($allModulesDatabase.CmdletToModuleMapping.Count))" -ForegroundColor Green
    } else {
        Write-Host "  âœ— La base de donnÃ©es ne contient pas de mappings de cmdlets" -ForegroundColor Red
    }
    
    if ($allModulesDatabase.TypeToModuleMapping.Count -eq 0) {
        Write-Host "  âœ“ La base de donnÃ©es ne contient pas de mappings de types (comme demandÃ©)" -ForegroundColor Green
    } else {
        Write-Host "  âœ— La base de donnÃ©es contient des mappings de types alors qu'ils ne devraient pas Ãªtre inclus" -ForegroundColor Red
    }
    
    if ($allModulesDatabase.VariableToModuleMapping.Count -eq 0) {
        Write-Host "  âœ“ La base de donnÃ©es ne contient pas de mappings de variables (comme demandÃ©)" -ForegroundColor Green
    } else {
        Write-Host "  âœ— La base de donnÃ©es contient des mappings de variables alors qu'ils ne devraient pas Ãªtre inclus" -ForegroundColor Red
    }
} else {
    Write-Host "  âœ— Ã‰chec de la crÃ©ation de la base de donnÃ©es sans spÃ©cifier de modules" -ForegroundColor Red
}
