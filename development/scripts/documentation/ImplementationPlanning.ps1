...
"@
        ...
        function New-ImplementationPlanReport {
            ...
            # Calculer les statistiques par statut (utilisées dans le rapport)
            $report.TaskStatusDistribution = @{
                "Completed"   = $completedTasks
                "In Progress" = ($tasks | Where-Object { $_.Status -eq "In Progress" } | Measure-Object).Count
                "Not Started" = ($tasks | Where-Object { $_.Status -eq "Not Started" } | Measure-Object).Count
                "Blocked"     = ($tasks | Where-Object { $_.Status -eq "Blocked" } | Measure-Object).Count
            }

            # Les statistiques sont maintenant stockées dans $report.TaskStatusDistribution
