...
<#
.SYNOPSIS
    Installe un hook pre-commit Git pour organiser automatiquement les scripts du manager.
.DESCRIPTION
    Ce script installe un hook pre-commit Git qui vÃ©rifie si des scripts PowerShell
    ont Ã©tÃ© ajoutÃ©s Ã  la racine du dossier manager et les organise automatiquement.
.PARAMETER Force
    Force l'installation du hook sans demander de confirmation.
.EXAMPLE
    .\Install-ManagerPreCommitHook.ps1 -Force
.NOTES
    Version: 1.0.0
...
#!/bin/sh
#
# Pre-commit hook pour organiser les scripts du manager
#

# VÃ©rifier si des fichiers PowerShell ont Ã©tÃ© ajoutÃ©s Ã  la racine du dossier manager
MANAGER_DIR="development/scripts/mode-manager"
ADDED_PS_FILES=$(git diff --cached --name-only --diff-filter=A | grep -E "^$MANAGER_DIR/[^/]+\.(ps1|psm1|psd1)$")

if [ -n "$ADDED_PS_FILES" ]; then
    echo "Des fichiers PowerShell ont Ã©tÃ© ajoutÃ©s Ã  la racine du dossier manager:"
    echo "$ADDED_PS_FILES"
    
    # ExÃ©cuter le script d'organisation
    echo "Organisation automatique des scripts..."
    powershell.exe -NoProfile -ExecutionPolicy Bypass -File "$MANAGER_DIR/organization/Organize-ManagerScripts.ps1" -Force
...
Write-Log "Le hook organisera automatiquement les scripts PowerShell ajoutÃ©s Ã  la racine des dossiers manager et maintenance." -Level "INFO"
