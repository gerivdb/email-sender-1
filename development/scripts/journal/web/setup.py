...
def setup_vscode():
    """Configure VS Code."""
    try:
        subprocess.run([sys.executable, "development/scripts/python/journal/journal_vscode.py", "setup"], check=True)
...
def setup_git_hooks():
    ...
    try:
        subprocess.run([sys.executable, "development/scripts/python/journal/setup_git_hooks.py"], check=True)
...
def migrate_journal():
    ...
    if journal_file.exists():
        try:
            subprocess.run([sys.executable, "development/scripts/python/journal/migrate_journal.py"], check=True)
...
def build_search_index():
    ...
    try:
        subprocess.run([sys.executable, "development/scripts/python/journal/journal_search_simple.py", "--rebuild"], check=True)
...
def build_rag_index():
    ...
    try:
        subprocess.run([sys.executable, "development/scripts/python/journal/journal_rag_simple.py", "--rebuild", "--export"], check=True)
...
def create_sample_entry():
    ...
    if not list(entries_dir.glob("*.md")):
        try:
            subprocess.run([
                sys.executable,
                "development/scripts/python/journal/journal_entry.py",
...
def setup_all():
    ...
    print("\nUtilisation:")
    print("- Pour créer une nouvelle entrée: python development/scripts/python/journal/journal_vscode.py new")
    print("- Pour rechercher dans le journal: python development/scripts/python/journal/journal_vscode.py search")
    print("- Pour interroger le système RAG: python development/scripts/python/journal/journal_rag.py --query \"votre requête\"")
