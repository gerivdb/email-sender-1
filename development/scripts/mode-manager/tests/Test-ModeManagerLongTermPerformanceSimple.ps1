# Tests de performance à long terme simplifiés pour le mode manager
...
for ($i = 1; $i -le 100; $i++) {
    $roadmapContent += "## Tâche $i`n`n"
    for ($j = 1; $j -le 5; $j++) {
        $roadmapContent += "### Sous-tâche $i.$j`n`n"
        for ($k = 1; $k -le 3; $k++) {
            $roadmapContent += "- [ ] Élément $i.$j.$k`n"
            $roadmapContent += "  - Description de l'élément $i.$j.$k`n"
            $roadmapContent += "  - Détails supplémentaires pour l'élément $i.$j.$k`n`n"
        }
    }
}
...
if ($secondHalfAverage -gt $firstHalfAverage * 1.1) {
    Write-Host "Test 1 échoué: Les performances se dégradent au fil du temps" -ForegroundColor Red
} else {
    Write-Host "Test 1 réussi: Les performances ne se dégradent pas au fil du temps" -ForegroundColor Green
}
