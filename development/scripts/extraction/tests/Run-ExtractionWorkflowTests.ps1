# Run-ExtractionWorkflowTests.ps1
# Script pour exécuter les tests de workflow d'extraction et stockage
...
$resultsFile = Join-Path -Path $resultsDir -ChildPath "ExtractionWorkflowTests_Results.txt"
...
Set-Content -Path $resultsFile -Value "# Résultats des tests de workflow d'extraction et stockage`r`n"
Add-Content -Path $resultsFile -Value "Date d'exécution : $(Get-Date)`r`n"

# Trouver tous les tests de workflow d'extraction et stockage
$extractionWorkflowTests = @(
...
# Exécuter les tests
$totalTests = $extractionWorkflowTests.Count
$passedTests = 0
$failedTests = 0

Add-Content -Path $resultsFile -Value "## Tests exécutés`r`n"

foreach ($test in $extractionWorkflowTests) {
...
# Afficher le résumé
Write-Host "`nRésumé des tests de workflow d'extraction et stockage :" -ForegroundColor $infoColor
...
if ($failedTests -eq 0) {
    Write-Host "Tous les tests de workflow d'extraction et stockage ont réussi!" -ForegroundColor $successColor
    exit 0
} else {
    Write-Host "Certains tests de workflow d'extraction et stockage ont échoué. Consultez le fichier de résultats pour plus de détails : $resultsFile" -ForegroundColor $errorColor
    exit 1
}
