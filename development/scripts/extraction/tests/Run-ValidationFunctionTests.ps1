# Run-ValidationFunctionTests.ps1
# Script pour exécuter les tests des fonctions de validation
...
$resultsFile = Join-Path -Path $resultsDir -ChildPath "ValidationFunctionTests_Results.txt"
...
Set-Content -Path $resultsFile -Value "# Résultats des tests des fonctions de validation`r`n"
Add-Content -Path $resultsFile -Value "Date d'exécution : $(Get-Date)`r`n"

# Trouver tous les tests des fonctions de validation
$validationTests = @(
...
# Afficher le résumé
Write-Host "`nRésumé des tests des fonctions de validation :" -ForegroundColor $infoColor
...
if ($failedTests -eq 0) {
    Write-Host "Tous les tests des fonctions de validation ont réussi!" -ForegroundColor $successColor
    exit 0
} else {
    Write-Host "Certains tests des fonctions de validation ont échoué. Consultez le fichier de résultats pour plus de détails : $resultsFile" -ForegroundColor $errorColor
    exit 1
}
