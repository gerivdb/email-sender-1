# Run-SerializationFunctionTests.ps1
# Script pour exécuter les tests des fonctions de sérialisation
...
$resultsFile = Join-Path -Path $resultsDir -ChildPath "SerializationFunctionTests_Results.txt"
...
Set-Content -Path $resultsFile -Value "# Résultats des tests des fonctions de sérialisation`r`n"
Add-Content -Path $resultsFile -Value "Date d'exécution : $(Get-Date)`r`n"

# Trouver tous les tests des fonctions de sérialisation
$serializationTests = @(
...
# Exécuter les tests
$totalTests = $serializationTests.Count
$passedTests = 0
$failedTests = 0

Add-Content -Path $resultsFile -Value "## Tests exécutés`r`n"

foreach ($test in $serializationTests) {
...
# Afficher le résumé
Write-Host "`nRésumé des tests des fonctions de sérialisation :" -ForegroundColor $infoColor
...
if ($failedTests -eq 0) {
    Write-Host "Tous les tests des fonctions de sérialisation ont réussi!" -ForegroundColor $successColor
    exit 0
} else {
    Write-Host "Certains tests des fonctions de sérialisation ont échoué. Consultez le fichier de résultats pour plus de détails : $resultsFile" -ForegroundColor $errorColor
    exit 1
}
