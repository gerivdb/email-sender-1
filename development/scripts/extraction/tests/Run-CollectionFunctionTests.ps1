# Run-CollectionFunctionTests.ps1
# Script pour exécuter les tests des fonctions de collection
...
$resultsFile = Join-Path -Path $resultsDir -ChildPath "CollectionFunctionTests_Results.txt"
...
Set-Content -Path $resultsFile -Value "# Résultats des tests des fonctions de collection`r`n"
Add-Content -Path $resultsFile -Value "Date d'exécution : $(Get-Date)`r`n"

# Trouver tous les tests des fonctions de collection
$collectionTests = @(
...
# Exécuter les tests
$totalTests = $collectionTests.Count
$passedTests = 0
$failedTests = 0
...
# Afficher le résumé
Write-Host "`nRésumé des tests des fonctions de collection :" -ForegroundColor $infoColor
...
if ($failedTests -eq 0) {
    Write-Host "Tous les tests des fonctions de collection ont réussi!" -ForegroundColor $successColor
    exit 0
} else {
    Write-Host "Certains tests des fonctions de collection ont échoué. Consultez le fichier de résultats pour plus de détails : $resultsFile" -ForegroundColor $errorColor
    exit 1
}
