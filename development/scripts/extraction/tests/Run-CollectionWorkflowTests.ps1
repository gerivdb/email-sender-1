# Run-CollectionWorkflowTests.ps1
# Script pour exécuter les tests de workflow de collection et filtrage
...
$resultsFile = Join-Path -Path $resultsDir -ChildPath "CollectionWorkflowTests_Results.txt"
...
Set-Content -Path $resultsFile -Value "# Résultats des tests de workflow de collection et filtrage`r`n"
Add-Content -Path $resultsFile -Value "Date d'exécution : $(Get-Date)`r`n"

# Trouver tous les tests de workflow de collection et filtrage
...
# Exécuter les tests
$totalTests = $collectionWorkflowTests.Count
$passedTests = 0
$failedTests = 0

Add-Content -Path $resultsFile -Value "## Tests exécutés`r`n"

foreach ($test in $collectionWorkflowTests) {
...
# Afficher le résumé
Write-Host "`nRésumé des tests de workflow de collection et filtrage :" -ForegroundColor $infoColor
...
if ($failedTests -eq 0) {
    Write-Host "Tous les tests de workflow de collection et filtrage ont réussi!" -ForegroundColor $successColor
    exit 0
} else {
    Write-Host "Certains tests de workflow de collection et filtrage ont échoué. Consultez le fichier de résultats pour plus de détails : $resultsFile" -ForegroundColor $errorColor
    exit 1
}
