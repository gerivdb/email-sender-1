# Run-ValidationWorkflowTests.ps1
# Script pour exécuter les tests de workflow de validation et correction
...
$resultsFile = Join-Path -Path $resultsDir -ChildPath "ValidationWorkflowTests_Results.txt"
...
Set-Content -Path $resultsFile -Value "# Résultats des tests de workflow de validation et correction`r`n"
Add-Content -Path $resultsFile -Value "Date d'exécution : $(Get-Date)`r`n"

# Trouver tous les tests de workflow de validation et correction
$validationWorkflowTests = @(
...
# Exécuter les tests
$totalTests = $validationWorkflowTests.Count
$passedTests = 0
$failedTests = 0

Add-Content -Path $resultsFile -Value "## Tests exécutés`r`n"

foreach ($test in $validationWorkflowTests) {
...
# Afficher le résumé
Write-Host "`nRésumé des tests de workflow de validation et correction :" -ForegroundColor $infoColor
...
if ($failedTests -eq 0) {
    Write-Host "Tous les tests de workflow de validation et correction ont réussi!" -ForegroundColor $successColor
    exit 0
} else {
    Write-Host "Certains tests de workflow de validation et correction ont échoué. Consultez le fichier de résultats pour plus de détails : $resultsFile" -ForegroundColor $errorColor
    exit 1
}
