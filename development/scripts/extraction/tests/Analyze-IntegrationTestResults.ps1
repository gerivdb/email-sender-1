# Analyze-IntegrationTestResults.ps1
# Script pour analyser les résultats des tests d'intégration
...
$analysisFile = Join-Path -Path $analysisDir -ChildPath "IntegrationTestResults_Analysis.md"

# Initialiser le fichier d'analyse
Set-Content -Path $analysisFile -Value "# Analyse des résultats des tests d'intégration`r`n"
...
# Trouver tous les fichiers de résultats des tests d'intégration
...
if ($allSuccess) {
    Add-Content -Path $analysisFile -Value "`r`n**Tous les tests d'intégration ont réussi!**"
    Write-Host "Tous les tests d'intégration ont réussi!" -ForegroundColor $successColor
} else {
    Add-Content -Path $analysisFile -Value "`r`n**Certains tests d'intégration ont échoué ou sont en erreur.**"
    Write-Host "Certains tests d'intégration ont échoué ou sont en erreur. Consultez le fichier d'analyse pour plus de détails : $analysisFile" -ForegroundColor $errorColor
}

# Afficher le résumé
Write-Host "`nRésumé global des tests d'intégration :" -ForegroundColor $infoColor
