...
# Test de la fonction New-ExtractedInfo avec paramètres par défaut
...
$tests = @(
    ...
    @{ Test = "La source est une chaîne vide"; Condition = $info.Source -eq "" }
    @{ Test = "L'extracteur est une chaîne vide"; Condition = $info.ExtractorName -eq "" }
