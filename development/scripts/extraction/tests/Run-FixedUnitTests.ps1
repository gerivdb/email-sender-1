# Run-FixedUnitTests.ps1
# Script pour réexécuter les tests unitaires corrigés
...
function Restart-UnitTests {
    ...
    Set-Content -Path $resultsFile -Value "# Résultats des tests unitaires corrigés : $Category`r`n"
    ...
    # Afficher le résumé
    Write-Host "`nRésumé des tests unitaires corrigés ($Category) :" -ForegroundColor $infoColor
...
# Définir les catégories de tests unitaires et leurs scripts associés
$unitTestCategories = @(
    @{
    ...
    @{
        Category = "MetadataFunction"
        Tests = @(
            "Test-AddExtractedInfoMetadata.ps1",
            "Test-GetExtractedInfoMetadata.ps1",
            "Test-RemoveExtractedInfoMetadata.ps1",
            "Test-ClearExtractedInfoMetadata.ps1"
        )
    },
    @{
        Category = "CollectionFunction"
        Tests = @(
            "Test-NewExtractedInfoCollection.ps1",
            "Test-AddExtractedInfoToCollection.ps1",
            "Test-GetExtractedInfoFromCollection.ps1",
            "Test-RemoveExtractedInfoFromCollection.ps1",
            "Test-GetExtractedInfoCollectionStatistics.ps1"
        )
    },
    @{
        Category = "SerializationFunction"
        Tests = @(
    ...
    @{
        Category = "ValidationFunction"
        Tests = @(
            "Test-ExtractedInfoValidation.ps1",
...
# Réexécuter les tests unitaires
$allResults = @()
$totalTests = 0
$totalPassedTests = 0
$totalFailedTests = 0
...
Write-Host "`nRésumé global des tests unitaires corrigés :" -ForegroundColor $infoColor
Write-Host "  Total des tests : $totalTests" -ForegroundColor $infoColor
...
$globalResultsFile = Join-Path -Path $fixedResultsDir -ChildPath "UnitTests_Fixed_Summary.md"
Set-Content -Path $globalResultsFile -Value "# Résumé global des tests unitaires corrigés`r`n"
...
if ($allSuccess) {
    Write-Host "Tous les tests unitaires corrigés ont réussi!" -ForegroundColor $successColor
    exit 0
} else {
