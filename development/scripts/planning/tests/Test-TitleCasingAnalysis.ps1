...
"@
    ...
    Context "Extraction des titres" {
        It "Devrait extraire tous les titres du document" {
            ...
            # V̩rifier que le r̩sultat contient le bon nombre de titres
            $result.TotalTitles | Should -Be 9
        }
    }

    Context "Analyse des styles de casse" {
        It "Devrait identifier correctement les diff̩rents styles de casse" {
            $result = & $scriptPath -FilePath $testFilePath -OutputPath $testOutputPath -IncludeExamples $true
            
            # V̩rifier que tous les styles de casse sont d̩tect̩s
        ...
        It "Devrait calculer correctement les statistiques par style de casse" {
            ...
            # V̩rifier les comptages de styles
    ...
    Context "Calcul de la coh̩rence" {
        ...
        It "Devrait calculer correctement la coh̩rence par niveau" {
            $result = & $scriptPath -FilePath $testFilePath -OutputPath $testOutputPath -IncludeExamples $true
            
            # V̩rifier la coh̩rence du niveau 1
            ...
            # V̩rifier la coh̩rence du niveau 2
    ...
    Context "G̩n̩ration du rapport" {
        ...
        It "Devrait inclure ou exclure les exemples selon le param̬tre" {
            # Test avec exemples
