...
function Start-Tests {
    ...
    foreach ($testCase in $TestCases) {
        ...
        if ($OutputFormat -eq "JSON") {
            ...
            $valuesCount = $resultObj.Count
        } else {
            # Pour les autres formats, compter les occurrences de "Value:" dans le résultat
            $valuesCount = ([regex]::Matches($result, "Value:")).Count
        }
        ...
        if ($valuesCount -gt 0) {
            Write-Host "  Résultat: $valuesCount valeurs trouvées" -ForegroundColor Green
            $passedTests++
        } else {
