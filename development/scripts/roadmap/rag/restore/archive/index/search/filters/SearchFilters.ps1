...
# Fonction pour combiner des filtres
function Merge-Filters {
    ...
    process {
        ...
        foreach ($filter in $Filters) {
            $filterResult = & $filter $Document

            # Appliquer l'operateur logique
            if ($LogicalOperator -eq "AND" -and -not $filterResult) {
                return
            } elseif ($LogicalOperator -eq "OR" -and $filterResult) {
                return $Document
            }
        }

        # Si l'operateur est AND, tous les filtres doivent correspondre
        # Si l'operateur est OR, au moins un filtre doit correspondre
