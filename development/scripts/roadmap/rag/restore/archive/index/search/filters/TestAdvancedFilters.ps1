...
$filtersPath = Join-Path -Path $scriptPath -ChildPath "AdvancedSearchFilters.ps1"

if (Test-Path -Path $filtersPath) {
    . $filtersPath
} else {
    Write-Error "Le fichier AdvancedSearchFilters.ps1 est introuvable."
    exit 1
}
...
$filteredDocuments = $documents | 
    Filter-ByType -IncludeTypes @("document", "pdf", "email") | 
