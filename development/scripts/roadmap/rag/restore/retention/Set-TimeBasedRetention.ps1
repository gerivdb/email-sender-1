# Set-TimeBasedRetention.ps1
# Script pour configurer et appliquer la rétention basée sur le temps pour les points de restauration
# Version: 1.0
# Date: 2025-05-15
...
# Fonction pour configurer la rétention basée sur le temps
function Set-TimeBasedRetention {
    ...
    # Mettre à jour ou ajouter la politique de rétention basée sur le temps
    if (-not $config.policies.ContainsKey("time_based")) {
        $config.policies.time_based = @{}
    }
    
    if (-not $config.policies.time_based.ContainsKey($normalizedType)) {
        $config.policies.time_based.$normalizedType = @{}
    }
    
    $config.policies.time_based.$normalizedType.retention_days = $RetentionDays
    $config.policies.time_based.$normalizedType.last_updated = (Get-Date).ToString("o")
    ...
    Write-Log "Time-based retention policy configured: $RetentionDays days for $RestorePointType restore points" -Level "Info"
...
function Apply-TimeBasedRetention {
    ...
    if ($WhatIf) {
        ...
        Write-Log "Applied time-based retention: $deletedCount restore points deleted, $errorCount errors" -Level "Info"
    }
