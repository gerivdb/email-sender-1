...
function Install-PythonDependencies {
    ...
    if ($FixCompatibility) {
        # Liste des dépendances avec versions spécifiques pour assurer la compatibilité
        Write-Log "Installation des dépendances avec versions spécifiques pour assurer la compatibilité..." -Level "Info"
        ...
        foreach ($dependency in $compatibleDependencies) {
        ...
        # Liste des dépendances standard
        $dependencies = @(
    ...
    try {
        ...
        if ($LASTEXITCODE -eq 0) {
            Write-Log "Dépendances Python installées et vérifiées avec succès." -Level "Success"
            return $true
        } else {
            Write-Log "Erreur lors de la vérification des dépendances: $testImport" -Level "Warning"

            if (-not $FixCompatibility) {
                Write-Log "Tentative de réinstallation avec des versions compatibles..." -Level "Info"
                return Install-PythonDependencies -FixCompatibility
            } else {
                Write-Log "Impossible de résoudre les problèmes de compatibilité." -Level "Error"
                return $false
            }
        }
    } catch {
        Write-Log "Erreur lors de la vérification des dépendances: $_" -Level "Warning"
...
function Install-Dependencies {
    ...
    $pythonDependenciesInstalled = Install-PythonDependencies -FixCompatibility:$Force
