        ...
        if ($results.Count -eq $test.ExpectedCount) {
            if ($test.ExpectedCount -eq 1) {
                if ($results[0].NumericValue -eq $test.ExpectedValue) {
                    ...
                    Write-Host "[ERREUR] Detection dans '$($test.Text)' => $($results[0].NumericValue) (attendu: $($test.ExpectedValue))" -ForegroundColor Red
                    $phraseFailCount++
                }
            } else {
                $success = $true
                for ($i = 0; $i -lt $test.ExpectedCount; $i++) {
                    if ($results[$i].NumericValue -ne $test.ExpectedValues[$i]) {
                        $success = $false
                        break
                    }
                }
                ...
                if ($success) {
                    Write-Host "[OK] Detection multiple dans '$($test.Text)' => $($results | ForEach-Object { $_.NumericValue })" -ForegroundColor Green
                    $phraseSuccessCount++
                } else {
            ...
            Write-Host "[ERREUR] Detection dans '$($test.Text)' => $($results.Count) resultats (attendu: $($test.ExpectedCount))" -ForegroundColor Red
            $phraseFailCount++
        }
    }
