...
<#
.SYNOPSIS
    Adaptateur pour int̩grer Convert-FileFormat.Tests.Simplified.ps1 avec les tests r̩els.
...
# Cr̩er un adaptateur pour la fonction Convert-FileFormat
function Convert-FileFormat {
    ...
    process {
        ...
        if (Get-Command -Name "Convert-FileFormat" -ErrorAction SilentlyContinue) {
            # Appeler la fonction r̩elle avec les param̬tres adapt̩s
            $result = & "Convert-FileFormat" @PSBoundParameters
            
            # Retourner le r̩sultat
            return $result
        }
        else {
            Write-Error "La fonction Convert-FileFormat n'existe pas dans le module."
            return $null
        }
    }
}
...
Export-ModuleMember -Function Convert-FileFormat

# Ex̩cuter les tests simplifi̩s avec l'adaptateur
$simplifiedTestPath = Join-Path -Path $PSScriptRoot -ChildPath "..\\Convert-FileFormat.Tests.Simplified.ps1"
