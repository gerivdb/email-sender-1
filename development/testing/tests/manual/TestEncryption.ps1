...
if (-not (Test-Path -Path $encryptionUtilsPath)) {
    Write-Error "Le module EncryptionUtils.ps1 n'existe pas au chemin spécifié."
    exit 1
}
...
Write-Host "Chaîne déchiffrée : $decryptedString" -ForegroundColor Cyan
...
Write-Host "Résultat du chiffrement : $encryptResult" -ForegroundColor $(if ($encryptResult) { "Green" } else { "Red" })
...
Write-Host "Résultat du déchiffrement : $decryptResult" -ForegroundColor $(if ($decryptResult) { "Green" } else { "Red" })
...
Write-Host "Résultat de la vérification : $($verifyResult.IsValid)" -ForegroundColor $(if ($verifyResult.IsValid) { "Green" } else { "Red" })
