...
<#
.SYNOPSIS
    Adaptateur pour int̩grer Show-FormatDetectionResults.Tests.Simplified.ps1 avec les tests r̩els.
...
# Cr̩er un adaptateur pour la fonction Show-FormatDetectionResults
function Show-FormatDetectionResults {
    ...
    process {
        ...
        if (Get-Command -Name "Show-FormatDetectionResults" -ErrorAction SilentlyContinue) {
            # Appeler la fonction r̩elle avec les param̬tres adapt̩s
            $result = & "Show-FormatDetectionResults" @PSBoundParameters
            
            # Retourner le r̩sultat
            return $result
        }
        else {
            Write-Error "La fonction Show-FormatDetectionResults n'existe pas dans le module."
            return $null
        }
    }
}
...
$simplifiedTestPath = Join-Path -Path $PSScriptRoot -ChildPath "..\\Show-FormatDetectionResults.Tests.Simplified.ps1"
