...
<#
.SYNOPSIS
    Adaptateur pour int̩grer Test-DetectedFileFormat.Tests.Simplified.ps1 avec les tests r̩els.
...
# Cr̩er un adaptateur pour la fonction Test-DetectedFileFormat
function Test-DetectedFileFormat {
    [CmdletBinding()]
    param (
    ...
    process {
        ...
        if (Get-Command -Name "Detect-FileFormat" -ErrorAction SilentlyContinue) {
            ...
            $result = & "Detect-FileFormat" @PSBoundParameters
...
Export-ModuleMember -Function Test-DetectedFileFormat

# Ex̩cuter les tests simplifi̩s avec l'adaptateur
$simplifiedTestPath = Join-Path -Path $PSScriptRoot -ChildPath "..\\Test-DetectedFileFormat.Tests.Simplified.ps1"
