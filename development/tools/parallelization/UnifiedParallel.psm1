            ...
            try {
                ...
                foreach ($foreachResult in $foreachResults) {
                    ...
                    if (-not $foreachResult.Success) {
                        [void]$errors.Add($foreachResult.Error)
                    }
                    $processedItems++
                        ...
                        if ($runspace.Handle.IsCompleted) {
                            ...
                            if ($runspaceResult) {
                                ...
                                if (-not $runspaceResult.Success) {
                                    [void]$errors.Add($runspaceResult.Error)
                                }
                            }
...
function Wait-ForCompletedRunspace {
    ...
    process {
        ...
        # Créer un nouveau ArrayList avec les résultats
        $finalResult = New-Object System.Collections.ArrayList

        # Ajouter chaque élément individuellement pour s'assurer que nous avons un ArrayList
        foreach ($item in $completedRunspaces) {
            [void]$finalResult.Add($item)
        }
        ...
        if (-not $WaitForAll -and $finalResult.Count -gt 1) {
            ...
            $singleResult = New-Object System.Collections.ArrayList
            if ($finalResult.Count -gt 0) {
                [void]$singleResult.Add($finalResult[0])
            }

            # Vérifier explicitement que nous avons un ArrayList avec un seul élément
            Write-Verbose "Type de retour final: $($singleResult.GetType().FullName), Count: $($singleResult.Count)"

            # Retourner explicitement l'ArrayList en l'encapsulant dans un PSCustomObject
            $resultObject = [PSCustomObject]@{
                Results = $singleResult
            }
        } else {
            # Vérifier explicitement que nous avons un ArrayList
        ...
        # Ajouter une méthode pour accéder à l'ArrayList
...
Export-ModuleMember -Function Initialize-UnifiedParallel, Clear-UnifiedParallel, Invoke-UnifiedParallel, Get-OptimalThreadCount, Wait-ForCompletedRunspace, Invoke-RunspaceProcessor, Get-ModuleInitialized, Set-ModuleInitialized, Get-ModuleConfig, Set-ModuleConfig
