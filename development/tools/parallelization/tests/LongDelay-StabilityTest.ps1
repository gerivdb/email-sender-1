# Test de stabilité pour Wait-ForCompletedRunspace avec des délais très longs
# Ce script teste la capacité de Wait-ForCompletedRunspace à gérer des délais très longs (>500ms)
...
Write-Host "Test de stabilité pour Wait-ForCompletedRunspace avec délais longs" -ForegroundColor Yellow
...
Write-Host "Création de $runspaceCount runspaces avec délais très longs..." -ForegroundColor Cyan

# Créer les runspaces avec des délais très longs
...
for ($i = 0; $i -lt $runspaceCount; $i++) {
    ...
    # Ajouter un script simple avec délai très long
...
Write-Host "Attente de $runspaceCount runspaces avec délais très longs..." -ForegroundColor Cyan
...
if ($success) {
    Write-Host "TEST DE STABILITÉ RÉUSSI!" -ForegroundColor Green
} else {
    Write-Host "TEST DE STABILITÉ ÉCHOUÉ!" -ForegroundColor Red
}
