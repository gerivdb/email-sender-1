<#
.SYNOPSIS
    Script pour exécuter les tests progressifs pour la fonction Invoke-RunspaceProcessor.

.DESCRIPTION
    Ce script exécute les tests progressifs pour la fonction Invoke-RunspaceProcessor
...
.EXAMPLE
    .\Run-InvokeRunspaceProcessorTests.ps1 -Phase P1

    Exécute les tests de phase 1 (tests basiques) pour la fonction Invoke-RunspaceProcessor.
...
$testScriptPath = Join-Path -Path $PSScriptRoot -ChildPath "Invoke-RunspaceProcessor.Progressive.Tests.ps1"
...
function Invoke-PhaseTests {
    ...
    Write-Host "Exécution des tests de phase $Phase pour Invoke-RunspaceProcessor..." -ForegroundColor Cyan
...
if ($Phase -eq 'All') {
    Write-Host "Exécution de tous les tests progressifs pour Invoke-RunspaceProcessor..." -ForegroundColor Green
