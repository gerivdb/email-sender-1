<#
.SYNOPSIS
    Script pour exécuter les tests progressifs pour la fonction Wait-ForCompletedRunspace.

.DESCRIPTION
    Ce script exécute les tests progressifs pour la fonction Wait-ForCompletedRunspace
...
.EXAMPLE
    .\Run-WaitForCompletedRunspaceTests.ps1 -Phase P1

    Exécute les tests de phase 1 (tests basiques) pour la fonction Wait-ForCompletedRunspace.
...
$testScriptPath = Join-Path -Path $PSScriptRoot -ChildPath "Wait-ForCompletedRunspace.Progressive.Tests.ps1"
...
function Invoke-PhaseTests {
    ...
    Write-Host "Exécution des tests de phase $Phase pour Wait-ForCompletedRunspace..." -ForegroundColor Cyan
...
if ($Phase -eq 'All') {
    Write-Host "Exécution de tous les tests progressifs pour Wait-ForCompletedRunspace..." -ForegroundColor Green
