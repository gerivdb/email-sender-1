...
:show_help
echo Utilisation du script capture-request.bat:
echo   capture-request [options]
echo.
echo Options:
...
echo.
echo Exemples:
echo   capture-request -request "Ajouter une fonctionnalite X"
echo   capture-request -request "Corriger le bug Y" -start -note "Urgent"
echo   capture-request -request "Ameliorer la performance" -category 3 -estimated "2-4"
goto end

:execute
powershell -ExecutionPolicy Bypass -File "%~dp0Capture-Request.ps1" %request% -Category %category% -EstimatedDays %estimated% %start% %note%
