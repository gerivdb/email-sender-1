...
try {
    ...
    Write-Host "âœ… Le fichier $mcpStandardPath est executable" -ForegroundColor Green
} catch {
    Write-Host "âŒ Erreur lors de l'execution de $mcpStandardPath : $_" -ForegroundColor Red
}

try {
    $mcpNotionOutput = & $mcpNotionPath 2>&1
    Write-Host "âœ… Le fichier $mcpNotionPath est executable" -ForegroundColor Green
} catch {
    Write-Host "âŒ Erreur lors de l'execution de $mcpNotionPath : $_" -ForegroundColor Red
}

try {
    $gatewayOutput = & $gatewayPath help 2>&1
    Write-Host "âœ… Le fichier $gatewayPath est executable" -ForegroundColor Green
} catch {
    Write-Host "âŒ Erreur lors de l'execution de $gatewayPath : $_" -ForegroundColor Red
}
