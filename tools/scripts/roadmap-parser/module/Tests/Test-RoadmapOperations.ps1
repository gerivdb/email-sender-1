#
# Test-RoadmapOperations.ps1
#
# Script pour tester les fonctions publiques de comptage d'op̩rations
#
...
Write-Host "D̩but des tests des fonctions publiques de comptage d'op̩rations..." -ForegroundColor Cyan
...
$counterName = "TestRoadmapOperationCounter"
...
$result = Measure-RoadmapOperations -Name "TestRoadmapMeasureOperations" -ScriptBlock {
    for ($i = 0; $i -lt 100; $i++) {
        Add-RoadmapOperationCount -Name "TestRoadmapMeasureOperations"
    }
...
$result = Measure-RoadmapOperations -Name "TestRoadmapMeasureOperationsWithParams" -ScriptBlock {
    param($count)
    
    for ($i = 0; $i -lt $count; $i++) {
        Add-RoadmapOperationCount -Name "TestRoadmapMeasureOperationsWithParams"
    }
...
$result = Measure-RoadmapOperations -Name "TestRoadmapMeasureOperationsWithPipeline" -ScriptBlock {
    process {
        for ($i = 0; $i -lt $_; $i++) {
            Add-RoadmapOperationCount -Name "TestRoadmapMeasureOperationsWithPipeline"
        }
...
# D̩finir un seuil
Set-RoadmapOperationThreshold -Name "TestRoadmapOperationThreshold" -Threshold 50

# Ex̩cuter plusieurs op̩rations
for ($i = 0; $i -lt 3; $i++) {
    $result = Measure-RoadmapOperations -Name "TestRoadmapOperationThreshold" -ScriptBlock {
        param($count)
        
        for ($j = 0; $j -lt $count; $j++) {
            Add-RoadmapOperationCount -Name "TestRoadmapOperationThreshold"
        }
...
$stats = Get-RoadmapOperationStatistics -Name "TestRoadmapOperationThreshold"
...
Write-Host "`nTests des fonctions publiques de comptage d'op̩rations termin̩s." -ForegroundColor Cyan
