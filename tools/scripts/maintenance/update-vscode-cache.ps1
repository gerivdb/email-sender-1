# Script pour mettre à jour les chemins dans le fichier mtime-cache.json de VS Code
# Ce script remplace les anciens chemins par les nouveaux chemins dans le fichier de cache VS Code

Write-Host "=== Mise à jour des chemins dans le fichier mtime-cache.json de VS Code ===" -ForegroundColor Cyan
...
if (-not (Test-Path $mtimeCachePath)) {
    Write-Host "❌ Le fichier mtime-cache.json n'existe pas à l'emplacement spécifié." -ForegroundColor Red
    exit 1
}
...
try {
    ...
    Write-Host "✅ Fichier mtime-cache.json lu avec succès." -ForegroundColor Green
}
catch {
...
try {
    ...
    Write-Host "❌ Erreur lors de la création de la copie de sauvegarde : $_" -ForegroundColor Red
    exit 1
}
...
if ($modified) {
    try {
        ...
        Write-Host "✅ Fichier mtime-cache.json mis à jour avec succès." -ForegroundColor Green
    }
    catch {
    ...
    Write-Host "ℹ️ Aucune modification nécessaire dans le fichier mtime-cache.json." -ForegroundColor Blue
}
