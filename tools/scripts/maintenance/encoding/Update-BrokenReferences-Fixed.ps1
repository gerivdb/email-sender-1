...
.DESCRIPTION
    Ce script utilise le rapport généré par Detect-BrokenReferences-Fixed.ps1 pour mettre à jour
...
.PARAMETER InputPath
    Chemin du fichier de rapport généré par Detect-BrokenReferences-Fixed.ps1.
    Par défaut: ..\..\D
...
function Convert-ToRelativePath {
    ...
    # Vérifier si les chemins sont vides
...
function Update-BrokenReferences {
    ...
    if (-not (Test-Path -Path $InputPath -ErrorAction SilentlyContinue)) {
        ...
        Write-Log "Exécutez d'abord Detect-BrokenReferences-Fixed.ps1 pour générer le rapport." -Level "ERROR"
        return $false
    }
    ...
    if (-not (Test-Path -Path $OutputDir -ErrorAction SilentlyContinue)) {
        try {
            ...
            Write-Log "Erreur lors de la création du dossier de sortie: $_" -Level "ERROR"
            return $false
        }
    }
