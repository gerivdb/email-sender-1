...
function Test-NewTestRepository {
    ...
    if (Test-Path -Path $scriptPaths["New-TestRepository"]) {
        # Cr̩er un bloc de script temporaire pour ̩viter l'ex̩cution automatique
        ...
        Write-Warning "Script New-TestRepository.ps1 non trouv̩"
        return
    }
...
function Test-NewTestPullRequest {
    ...
    if (Test-Path -Path $scriptPaths["New-TestPullRequest"]) {
        ...
        Write-Warning "Script New-TestPullRequest-Fixed.ps1 non trouv̩"
        return
    }
...
function Test-MeasurePRAnalysisPerformance {
    ...
    function Get-PullRequestInfo {
        ...
        Write-Host "Mock: Get-PullRequestInfo appel̩"
    ...
    function Invoke-PRAnalysis {
        ...
        Write-Host "Mock: Invoke-PRAnalysis appel̩"
        return [PSCustomObject]@{
    ...
    function New-PerformanceReport {
        ...
        Write-Host "Mock: New-PerformanceReport appel̩"
    ...
    if (Test-Path -Path $scriptPaths["Measure-PRAnalysisPerformance"]) {
        ...
        Write-Warning "Script Measure-PRAnalysisPerformance.ps1 non trouv̩"
        return
    }
...
function Test-StartPRTestSuite {
    ...
    if (Test-Path -Path $scriptPaths["Start-PRTestSuite"]) {
        ...
        Write-Warning "Script Start-PRTestSuite.ps1 non trouv̩"
        return
    }
    ...
    Test-Condition -Name "Initialize-TestRepository est appel̩ lorsque CreateRepository est true" -Condition {
        ...
        function Initialize-TestRepository {
            param($RepositoryPath, $Force)
            $script:called = $true
            Write-Host "Mock: Initialize-TestRepository appel̩ avec Force=$Force"
            return $true
        }
