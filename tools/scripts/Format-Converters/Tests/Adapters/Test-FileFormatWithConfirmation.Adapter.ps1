...
<#
.SYNOPSIS
    Adaptateur pour int̩grer Test-FileFormatWithConfirmation.Tests.Simplified.ps1 avec les tests r̩els.
...
# Cr̩er un adaptateur pour la fonction Test-FileFormatWithConfirmation
function Test-FileFormatWithConfirmation {
    ...
    process {
        ...
        if (Get-Command -Name "Detect-FileFormatWithConfirmation" -ErrorAction SilentlyContinue) {
            # Appeler la fonction r̩elle avec les param̬tres adapt̩s
            $result = & "Detect-FileFormatWithConfirmation" @PSBoundParameters
            
            # Retourner le r̩sultat
            return $result
        }
        else {
            Write-Error "La fonction Detect-FileFormatWithConfirmation n'existe pas dans le module."
            return $null
        }
    }
}
...
$simplifiedTestPath = Join-Path -Path $PSScriptRoot -ChildPath "..\\Test-FileFormatWithConfirmation.Tests.Simplified.ps1"
