<#
.SYNOPSIS
    Tests unitaires pour le script de génération de graphiques de tendances.
.DESCRIPTION
...
function New-TestTemplates {
    ...
    $JsonPath = Join-Path -Path $TemplatesDir -ChildPath "chart_templates.json"
...
Describe "Trend Charts Script Tests" {
    BeforeAll {
        ...
        $script:TemplatesFile = Join-Path -Path $TemplatesPath -ChildPath "chart_templates.json"
