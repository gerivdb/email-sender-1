...
class JournalRAG:
    def __init__(self):
        ...
        self.vector_file = self.rag_dir / "vectors.npy"
        self.chunks_file = self.rag_dir / "chunks.json"

        # Chargement ou création de l'index
        if self.index_file.exists() and self.vector_file.exists() and self.chunks_file.exists():
            self.load_index()
        else:
            self.build_index()
    ...
    def query(self, query_text, n=3):
        """Interroge le système RAG en utilisant la similarité cosinus."""
