...
<#
.SYNOPSIS
    Tests simplifiés pour le script Test-PRPerformanceRegression.ps1.
.DESCRIPTION
    Ce script contient des tests simplifiés pour le script Test-PRPerformanceRegression.ps1
...
if (-not (Test-Path -Path $scriptPath)) {
    throw "Script Test-PRPerformanceRegression.ps1 non trouvé à l'emplacement: $scriptPath"
}

# Tests Pester
Describe "Test-PRPerformanceRegression Tests Simplifiés" {
    ...
    Context "Vérification des paramètres" {
        It "Le script contient le paramètre CurrentResults" {
            $scriptContent = Get-Content -Path $scriptPath -Raw
            $scriptContent | Should -Match "param\s*\(.*\[Parameter.*\].*\$CurrentResults.*\)"
        }
        
        It "Le script contient le paramètre BaselineResults" {
            $scriptContent = Get-Content -Path $scriptPath -Raw
