...
function Invoke-MaintenanceStandardsTests {
    [CmdletBinding()]
    param (
        [Parameter(Mandatory = $false)]
        [string]$TestPath = (Join-Path -Path $PSScriptRoot -ChildPath "..\..\..\maintenance\standards\tests"),

        [Parameter(Mandatory = $false)]
        [string]$OutputPath = (Join-Path -Path $env:TEMP -ChildPath "TestOmnibus\Results\MaintenanceStandards"),
...
<head>
    <meta charset="UTF-8">
    <title>Rapport de tests - Maintenance Standards</title>
    <style>
...
Export-ModuleMember -Function Invoke-MaintenanceStandardsTests
